...
### ~~BUG-75 · motor do portão da Mansão era atravessável~~ · RESOLVIDO 22/08

**Evidência antes:** a varredura integrada `node tools/eval/map-check.mjs novos`
encontrou em `fy_mansao` um ponto alcançável em `(4,5; 34,5)` com **0,9 m** de
penetração. O motor visual do portão (`map_mansao.js`) tinha `collide:false`, então
o jogador podia entrar nele embora o raycast o enxergasse como sólido.

**Correção:** o motor agora usa o colisor de sua própria caixa. Depois:
`node tools/eval/map-check.mjs fy_mansao` → **MAP1 dentro 0**, submersos 0,
pior 0 m. A régua é `tools/eval/map-check.mjs`; o mutante
`--mutante=penetracao-injetada` continua garantindo que ela denuncia geometria
visível sem colisor.
