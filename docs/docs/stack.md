...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **123** importam Playwright, **41** importam gltf-transform e **5** importam meshoptimizer.
...
`mint-assets.json` registra **22 assets** gerados via Mint (3 `mint-model` · 7 `mint-asset-pack` · 12 `mint-chat`), cada um com `assetId`, `chatUrl` e notas do que deu errado na tentativa anterior.
