...
`mint-assets.json` registra **41 assets** gerados via Mint (19 `openrouter-image-concept` · 2 `openrouter-concept-tripo-blender-model` · 5 `imagegen-concept` · 7 `mint-model` · 2 `tripo3d-task` · 2 `openrouter-concept-meshy-model` · 4 `mint-asset-pack`), cada um com `assetId`, `chatUrl` e notas do que deu errado na tentativa anterior.
