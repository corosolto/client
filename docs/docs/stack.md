...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **144** importam Playwright, **62** importam gltf-transform e **7** importam meshoptimizer.
...
`mint-assets.json` registra **81 assets** gerados via Mint (19 `openrouter-image-concept` · 2 `openrouter-concept-tripo-blender-model` · 5 `imagegen-concept` · 30 `mint-model` · 2 `tripo3d-task` · 2 `openrouter-concept-meshy-model` · 8 `mint-asset-pack` · 9 `mint-chat` · 3 `local-derivative` · 1 `mint-text-to-3d`), cada um com `assetId`, `chatUrl` e notas do que deu errado na tentativa anterior.
