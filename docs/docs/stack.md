...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **143** importam Playwright, **52** importam gltf-transform e **10** importam meshoptimizer.
...
`mint-assets.json` registra **48 assets** gerados via Mint (11 `mint-model` · 12 `mint-asset-pack` · 5 `mint-text-to-3d` · 9 `mint-chat` · 4 `local-derivative` · 3 `mint-asset-pack-item` · 4 `replicate-gen`), cada um com `assetId`, `chatUrl` e notas do que deu errado na tentativa anterior.
