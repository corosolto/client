...
|---|---:|---|
| Código do jogo | 41.058 linhas em 72 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Scripts do arnês | 281 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Scripts de pipeline | 65 | `git ls-files 'tools/*.mjs' \| wc -l` |
...
**15 mapas registrados** — 2 abrem em rodadas e 13 em captura. `ctfMode` **abre** o mapa em captura, não prende: o jogador troca no menu (é a `MOD1`). Há 19 arquivos `map_*.js` em `public/js/` — arquivo no disco **não** implica mapa jogável.
