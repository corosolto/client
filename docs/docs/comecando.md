...
|---|---:|---|
| Código do jogo | 42.739 linhas em 78 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Scripts do arnês | 299 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Scripts de pipeline | 69 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Tarefas de entrada escritas | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Versão | `2.0.0-alpha.228` | `public/js/version.js` e `package.json` (batem) |
...
**16 mapas registrados** — 2 abrem em rodadas e 14 em captura. `ctfMode` **abre** o mapa em captura, não prende: o jogador troca no menu (é a `MOD1`). Há 20 arquivos `map_*.js` em `public/js/` — arquivo no disco **não** implica mapa jogável.
