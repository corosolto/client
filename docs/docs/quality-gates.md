...
- `tools/eval/invariants.mjs`: **2.269 linhas**, **65 identificadores de invariante declarados** (`put()`), dos quais **28** têm caminho de `skip()` declarado.
- O arnês inteiro são **156 scripts** em `tools/eval/` (`.mjs` + `.py`), mais **46 scripts** de pipeline em `tools/`.
