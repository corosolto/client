...
- `tools/eval/invariants.mjs`: **2.277 linhas**, **65 identificadores de invariante declarados** (`put()`), dos quais **28** têm caminho de `skip()` declarado.
- O arnês inteiro são **264 scripts** em `tools/eval/` (`.mjs` + `.py`), mais **75 scripts** de pipeline em `tools/`.
