...
|---|---:|---|
| Game code | 29,673 lines in 39 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **6,567** lines | `wc -l public/js/game.js` |
...
| Maps in the menu | 8 - 3 open in rounds, **5 in capture** | `MAPS` / `ctfMode` |
...
**8 registered maps** - 3 open in rounds and 5 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 9 `map_*.js` files on disk, so a file alone does **not** make a map playable.
