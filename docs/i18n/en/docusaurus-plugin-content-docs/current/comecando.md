...
|---|---:|---|
| Game code | 32,271 lines in 45 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **6,917** lines | `wc -l public/js/game.js` |
| `main.js` | 2,702 lines | `wc -l public/js/main.js` |
...
| Harness scripts | 197 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
| Maps in the menu | 13 - 3 open in rounds, **10 in capture** | `MAPS` / `ctfMode` |
...
| `treta_no_gelo` | Treta no Gelo | rounds | `map_gelo.js` | 250 |

**13 registered maps** - 3 open in rounds and 10 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 15 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **121 scripts**; the reason behind each one lives in `SCRIPTS.md`.
