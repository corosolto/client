...
|---|---:|---|
| Game code | 49,816 lines in 96 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props in GLB | 180 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 414 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **256 scripts**; the reason behind each one lives in `SCRIPTS.md`.
