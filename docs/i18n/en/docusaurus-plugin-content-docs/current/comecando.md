...
|---|---:|---|
| Game code | 39,579 lines in 67 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7,368** lines | `wc -l public/js/game.js` |
| `main.js` | 3,381 lines | `wc -l public/js/main.js` |
...
| Props in GLB | 124 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Maps in the registry | 14 | `MAPS` object in `maps.js` |
| Visual harnesses in HTML | 15 | `git ls-files 'public/*.html' \| wc -l` |
| Harness scripts | 264 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 62 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.225` | `public/js/version.js` and `package.json` (match) |
...
| Maps in the menu | 14 - 2 open in rounds, **12 in capture** | `MAPS` / `ctfMode` |
...
**14 registered maps** - 2 open in rounds and 12 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 16 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **186 scripts**; the reason behind each one lives in `SCRIPTS.md`.
