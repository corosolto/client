...
|---|---:|---|
| Game code | 41,104 lines in 73 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 284 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
|---|---|---|---|---:|
| `escadao` | Escadão (Morro) | **capture** | `map_escadao.js` | 1,317 |
...
**15 registered maps** - 2 open in rounds and 13 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 20 `map_*.js` files on disk, so a file alone does **not** make a map playable.
