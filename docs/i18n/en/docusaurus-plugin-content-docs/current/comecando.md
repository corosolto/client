...
|---|---:|---|
| Game code | 41,058 lines in 72 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7,374** lines | `wc -l public/js/game.js` |
| `main.js` | 3,385 lines | `wc -l public/js/main.js` |
...
| Props in GLB | 134 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Maps in the registry | 15 | `MAPS` object in `maps.js` |
| Visual harnesses in HTML | 15 | `git ls-files 'public/*.html' \| wc -l` |
| Harness scripts | 281 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 65 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.227` | `public/js/version.js` and `package.json` (match) |
...
| Maps in the menu | 15 - 2 open in rounds, **13 in capture** | `MAPS` / `ctfMode` |
...
|---|---|---|---|---:|
| `escadao` | Escadão (Morro) | **capture** | `map_escadao.js` | 1,295 |
...
| `lajes` | Lajes (Comunidade) | **capture** | `map_lajes_authored.js` | 389 |
...
**15 registered maps** - 2 open in rounds and 13 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 19 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **201 scripts**; the reason behind each one lives in `SCRIPTS.md`.
