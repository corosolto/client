...
|---|---:|---|
| Game code | 51,457 lines in 99 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 429 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **266 scripts**; the reason behind each one lives in `SCRIPTS.md`.
