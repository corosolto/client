...
|---|---:|---|
| Game code | 51,393 lines in 99 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 427 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **264 scripts**; the reason behind each one lives in `SCRIPTS.md`.
