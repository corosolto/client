...
|---|---:|---|
| Game code | 41,178 lines in 59 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| `amazonia` | Treta na Amazônia | **capture** | `map_amazonia.js` | 462 |

**18 registered maps** - 2 open in rounds and 16 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 20 `map_*.js` files on disk, so a file alone does **not** make a map playable.
