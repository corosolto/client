...
|---|---:|---|
| Game code | 54,296 lines in 111 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 544 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **390 scripts**; the reason behind each one lives in `SCRIPTS.md`.
