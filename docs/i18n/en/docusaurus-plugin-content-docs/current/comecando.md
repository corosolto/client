...
|---|---:|---|
| Game code | 44,522 lines in 90 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7,392** lines | `wc -l public/js/game.js` |
...
| Harness scripts | 329 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
|---|---|---|---|---:|
| `mansao` | Mansão do Joá | **capture** | `map_mansao.js` | 1,386 |
...
**17 registered maps** - 2 open in rounds and 15 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 22 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **216 scripts**; the reason behind each one lives in `SCRIPTS.md`.
