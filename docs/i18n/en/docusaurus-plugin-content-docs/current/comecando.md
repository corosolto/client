...
|---|---:|---|
| Game code | 50,796 lines in 97 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props in GLB | 181 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 424 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **260 scripts**; the reason behind each one lives in `SCRIPTS.md`.
