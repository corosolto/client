...
|---|---:|---|
| Game code | 37,728 lines in 58 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 232 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 62 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.222` | `public/js/version.js` and `package.json` (match) |
...
`package.json` has **157 scripts**; the reason behind each one lives in `SCRIPTS.md`.
