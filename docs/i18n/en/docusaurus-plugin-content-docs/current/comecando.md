...
|---|---:|---|
| Game code | 56,964 lines in 109 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **8,017** lines | `wc -l public/js/game.js` |
| `main.js` | 3,688 lines | `wc -l public/js/main.js` |
...
| Harness scripts | 573 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 100 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.300` | `public/js/version.js` and `package.json` (match) |
...
**18 registered maps** - 2 open in rounds and 16 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 25 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **351 scripts**; the reason behind each one lives in `SCRIPTS.md`.
