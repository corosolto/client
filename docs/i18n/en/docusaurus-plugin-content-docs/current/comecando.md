...
|---|---:|---|
| Game code | 51,111 lines in 97 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props in GLB | 186 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 426 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **262 scripts**; the reason behind each one lives in `SCRIPTS.md`.
