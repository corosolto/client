...
|---|---:|---|
| Game code | 50,751 lines in 97 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 423 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **259 scripts**; the reason behind each one lives in `SCRIPTS.md`.
