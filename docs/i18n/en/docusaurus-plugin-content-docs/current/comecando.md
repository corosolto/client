...
|---|---:|---|
| Game code | 41,248 lines in 73 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7,383** lines | `wc -l public/js/game.js` |
| `main.js` | 3,387 lines | `wc -l public/js/main.js` |
...
| Props in GLB | 136 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 282 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 66 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.225` | `public/js/version.js` and `package.json` **DIVERGE** |
...
|---|---|---|---|---:|
| `amazonia` | Treta na Amazônia | **capture** | `map_amazonia.js` | 1,101 |
...
**15 registered maps** - 2 open in rounds and 13 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 17 `map_*.js` files on disk, so a file alone does **not** make a map playable.
