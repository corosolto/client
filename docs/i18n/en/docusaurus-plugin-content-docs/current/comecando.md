...
|---|---:|---|
| Game code | 39,570 lines in 64 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7,356** lines | `wc -l public/js/game.js` |
...
| Props in GLB | 126 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
**14 registered maps** - 2 open in rounds and 12 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 18 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **178 scripts**; the reason behind each one lives in `SCRIPTS.md`.
