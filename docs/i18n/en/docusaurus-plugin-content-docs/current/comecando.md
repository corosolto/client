...
|---|---:|---|
| Game code | 42,972 lines in 64 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 307 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **191 scripts**; the reason behind each one lives in `SCRIPTS.md`.
