...
|---|---:|---|
| Game code | 56,881 lines in 109 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 571 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 100 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.298` | `public/js/version.js` and `package.json` (match) |
...
`package.json` has **349 scripts**; the reason behind each one lives in `SCRIPTS.md`.
