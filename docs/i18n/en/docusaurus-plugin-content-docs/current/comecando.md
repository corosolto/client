...
|---|---:|---|
| Game code | 51,438 lines in 99 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props in GLB | 183 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 428 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 80 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.288` | `public/js/version.js` and `package.json` (match) |
...
`package.json` has **265 scripts**; the reason behind each one lives in `SCRIPTS.md`.
