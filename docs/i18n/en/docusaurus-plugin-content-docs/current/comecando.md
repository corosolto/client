...
|---|---:|---|
| Game code | 41,878 lines in 59 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| `mansao` | Mansão do Joá | **capture** | `map_mansao.js` | 1,333 |
...
**17 registered maps** - 2 open in rounds and 15 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 21 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **189 scripts**; the reason behind each one lives in `SCRIPTS.md`.
