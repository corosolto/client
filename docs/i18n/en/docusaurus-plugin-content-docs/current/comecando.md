...
|---|---:|---|
| Game code | 37,428 lines in 57 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 227 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **153 scripts**; the reason behind each one lives in `SCRIPTS.md`.
