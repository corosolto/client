...
|---|---:|---|
| Game code | 40,631 lines in 57 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 269 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
| `fy_campomorro` | Campo do Morro | **capture** | `map_campomorro.js` | 670 |
| `fy_lajes` | Lajes (Comunidade) | **capture** | `map_lajes_authored.js` | 1,205 |
| `fy_corrego` | Córrego (Favela de SP) | **capture** | `map_corrego.js` | 1,535 |
...
`package.json` has **165 scripts**. Keys prefixed with `//` explain why the adjacent command exists.
