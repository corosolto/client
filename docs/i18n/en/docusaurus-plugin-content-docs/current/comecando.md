...
|---|---:|---|
| Game code | 40,566 lines in 56 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **6,976** lines | `wc -l public/js/game.js` |
...
| Props in GLB | 131 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 263 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
| `fy_campomorro` | Campo do Morro | **capture** | `map_campomorro.js` | 688 |
| `fy_lajes` | Lajes (Comunidade) | **capture** | `map_lajes_authored.js` | 1,209 |
| `fy_corrego` | Córrego (Favela de SP) | **capture** | `map_corrego.js` | 1,549 |
| `fy_mansao` | Mansão do Joá | **capture** | `map_mansao.js` | 823 |
...
| `penitenciaria` | Penitenciária da Treta | **capture** | `map_penitenciaria.js` | 262 |
...
`package.json` has **163 scripts**. Keys prefixed with `//` explain why the adjacent command exists.
