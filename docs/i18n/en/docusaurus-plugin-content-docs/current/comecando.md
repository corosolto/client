...
|---|---:|---|
| Game code | 54,380 lines in 111 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **8,045** lines | `wc -l public/js/game.js` |
| `main.js` | 3,633 lines | `wc -l public/js/main.js` |
...
| Props in GLB | 182 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 553 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 96 | `git ls-files 'tools/*.mjs' \| wc -l` |
...
| `piscina_treta` | Piscina da Treta | rounds | `map_piscina.js` | 875 |
| `loja_h` | Loja H (Estacionamento) | **capture** | `map_havan.js` | 2,014 |
...
| `posto_treta` | Posto da Treta | **capture** | `map_posto.js` | 507 |
| `upa_24h` | UPA 24h da Treta | **capture** | `map_upa.js` | 303 |
...
| `parque_treta` | Parque da Treta | **capture** | `map_parque.js` | 427 |
| `velho_oeste` | Sertão da Treta | **capture** | `map_velho_oeste.js` | 1,061 |
| `penitenciaria` | Penitenciária da Treta | **capture** | `map_penitenciaria.js` | 1,009 |

**17 registered maps** - 2 open in rounds and 15 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 25 `map_*.js` files on disk, so a file alone does **not** make a map playable.
...
`package.json` has **401 scripts**; the reason behind each one lives in `SCRIPTS.md`.
