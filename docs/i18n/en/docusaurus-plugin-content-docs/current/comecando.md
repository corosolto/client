...
|---|---:|---|
| Game code | 44,792 lines in 90 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 346 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 73 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.245` | `public/js/version.js` and `package.json` (match) |
...
`package.json` has **228 scripts**; the reason behind each one lives in `SCRIPTS.md`.
