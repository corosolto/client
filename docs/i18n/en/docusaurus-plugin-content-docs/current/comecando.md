...
|---|---:|---|
| Game code | 35,453 lines in 52 files | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7,083** lines | `wc -l public/js/game.js` |
| `main.js` | 2,707 lines | `wc -l public/js/main.js` |
...
| Maps in the menu | 14 - 3 open in rounds, **11 in capture** | `MAPS` / `ctfMode` |
...
**14 registered maps** - 3 open in rounds and 11 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 16 `map_*.js` files on disk, so a file alone does **not** make a map playable.
