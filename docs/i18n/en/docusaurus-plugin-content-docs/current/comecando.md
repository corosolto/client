...
|---|---:|---|
| Game code | 42,087 lines in 62 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 306 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
| `velho_oeste` | Sertão da Treta | **capture** | `map_velho_oeste.js` | 873 |
...
`package.json` has **193 scripts**; the reason behind each one lives in `SCRIPTS.md`.
