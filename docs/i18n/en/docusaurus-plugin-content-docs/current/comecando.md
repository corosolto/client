...
|---|---:|---|
| Game code | 41,126 lines in 60 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 291 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
| `fy_lajes` | Lajes (Comunidade) | **capture** | `map_lajes_authored.js` | 1,264 |
| `fy_corrego` | Córrego (Favela de SP) | **capture** | `map_corrego.js` | 1,345 |
| `fy_mansao` | Mansão do Joá | **capture** | `map_mansao.js` | 1,045 |
...
`package.json` has **179 scripts**; the reason behind each one lives in `SCRIPTS.md`.
