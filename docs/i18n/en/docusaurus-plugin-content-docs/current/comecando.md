...
|---|---:|---|
| Game code | 40,856 lines in 58 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props in GLB | 135 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Harness scripts | 275 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
`package.json` has **168 scripts**. Keys prefixed with `//` explain why the adjacent command exists.
