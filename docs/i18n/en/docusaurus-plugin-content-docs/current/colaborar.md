...
**10 human author identities** sign commits in this branch: `ruben-cytonic`, `Emerson Garrido`, `rubenmarcus`, `Ruben`, `Ruben Marcus`, `William Oliveira`, `Juan Versolato Lopes`, `matheusgb`, `daltonfontes`, `usantos`. Automated identities are excluded. A Git author name is not necessarily one unique person.
...
**8 registered maps** - 3 open in rounds and 5 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 10 `map_*.js` files on disk, so a file alone does **not** make a map playable.
