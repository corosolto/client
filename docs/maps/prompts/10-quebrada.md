# Quebrada / Rua do Baile — prompt de execução

Copie o bloco abaixo para Claude; ele também funciona no outro modelo se houver um único responsável pelo mapa.

```text
Você é o responsável por aprofundar visualmente Quebrada / Rua do Baile no CSBrasil, usando pesquisa real, acervo existente e Mint.gg quando necessário. Entregue implementação e evidência, não apenas planejamento.

CONTEXTO E FONTE
#530 astra/campo-morro-release-audit aberto trata campo dentro da Quebrada; ele NÃO substitui o mapa campomorro/#437.
...
ISOLAMENTO
Um builder por mapa. Não edite primary, checkout alheio nem a worktree integradora em paralelo. Parta do checkpoint integrado citado no índice e use worktree própria para este mapa; se já houver dono ativo, coordene a posse antes de editar. Seu escopo é public/js/map_quebrada.js, helpers exclusivos, assets exclusivos e gates/docs deste mapa. maps.js, game.js, main.js, look.js, mapprops.js, package.json e registros compartilhados exigem integração sequencial: entregue patch separado ao integrador. Não altere core, armas, HUD ou áudio como efeito colateral.

DIREÇÃO VISUAL
Rua brasileira comprida e viva, comércio, ônibus, bar e baile com variação coerente de fachadas, telhados, calçadas e grafites. Campo ao fundo deve pertencer ao bairro.
...
ACERVO E MINT.GG
Reusar casas, ônibus, frota, arquibancada e som existentes. Mint para módulos de comércio de esquina, coberturas e telhados faltantes; reusar varal_roupas.
...
Muretas, bancos, encostos, placar e navegação do #530; proteger rotas paralelas e travessias. Recuperar decals locais antes de julgar paredes vazias.
...
VERIFICAÇÃO
Use Node compatível com o projeto; nesta máquina PATH=/opt/homebrew/bin:$PATH. Confirme scripts disponíveis antes de executar: eval:mapcontrato; eval:spawn; gates campinho do #530 se incorporados. Acrescente contratos/spawns/CTF pertinentes. Um único browser pesado por vez no conjunto das frentes: reserve sua vez com o integrador. Capturas reais 1200×800, GLBs de fato carregados, ângulos repetíveis: rua, baile, bar, ponto de ônibus e campo interno; manter explícita a distinção dos dois mapas.
...
```text
Revise Quebrada / Rua do Baile independentemente. Primeiro abra as capturas reais antes/depois da execução acima; só depois leia o builder. Não receba a justificativa do autor como prova. Compare a direção visual descrita neste arquivo com referências reais, confirme GLBs carregados e examine rua, baile, bar, ponto de ônibus e campo interno; manter explícita a distinção dos dois mapas. Priorize: escala/encaixe, fidelidade ao lugar, cobertura verdadeira, rotas, leitura de inimigos e custo. Confira especialmente: Muretas, bancos, encostos, placar e navegação do #530; proteger rotas paralelas e travessias. Recuperar decals locais antes de julgar paredes vazias. Registre até cinco problemas concretos com imagem/câmera e arquivo:linha, separando regressão de aprofundamento ainda ausente. Não aceite pelo número de testes ou assets. Não edite simultaneamente com o builder; devolva correções delimitadas ou assuma o mapa somente após handoff explícito. Se precisar de Mint, confira o inventário antes para não duplicar geração. Declare o que não foi demonstrado.
```
