# Obras da Prefeitura — prompt de execução

Copie o bloco abaixo para GLM; ele também funciona no outro modelo se houver um único responsável pelo mapa.

```text
Você é o responsável por aprofundar visualmente Obras da Prefeitura no CSBrasil, usando pesquisa real, acervo existente e Mint.gg quando necessário. Entregue implementação e evidência, não apenas planejamento.

CONTEXTO E FONTE
PR #458 map2/obras, fechado, contém torre de andaime, bunker/container e miolo; consultar antes.
...
ISOLAMENTO
Um builder por mapa. Não edite primary, checkout alheio nem a worktree integradora em paralelo. Parta do checkpoint integrado citado no índice e use worktree própria para este mapa; se já houver dono ativo, coordene a posse antes de editar. Seu escopo é public/js/map_obras.js, helpers exclusivos, assets exclusivos e gates/docs deste mapa. maps.js, game.js, main.js, look.js, mapprops.js, package.json e registros compartilhados exigem integração sequencial: entregue patch separado ao integrador. Não altere core, armas, HUD ou áudio como efeito colateral.

DIREÇÃO VISUAL
Canteiro brasileiro legível: concreto em etapas, formas, escoramento, ferragem, andaimes, terra e tapumes. Dar escala e profundidade à obra sem esconder rotas em entulho.
...
ACERVO E MINT.GG
Recuperar kit do PR458 e conferir guindaste/entulho/barreiras/caminhão. Mint para formas modulares, escoramento e materiais empilhados que ainda faltarem.
...
Terreno ondulado, escavações, subidas, container/bunker e rotas. Não transformar andaime decorativo em promessa de caminho inexistente.
...
VERIFICAÇÃO
Use Node compatível com o projeto; nesta máquina PATH=/opt/homebrew/bin:$PATH. Confirme scripts disponíveis antes de executar: eval:mapcontrato; eval:spawn; gates específicos do PR458. Acrescente contratos/spawns/CTF pertinentes. Um único browser pesado por vez no conjunto das frentes: reserve sua vez com o integrador. Capturas reais 1200×800, GLBs de fato carregados, ângulos repetíveis: canteiro, estrutura por baixo, torre, container, acessos e vista da obra no bairro.
...
```text
Revise Obras da Prefeitura independentemente. Primeiro abra as capturas reais antes/depois da execução acima; só depois leia o builder. Não receba a justificativa do autor como prova. Compare a direção visual descrita neste arquivo com referências reais, confirme GLBs carregados e examine canteiro, estrutura por baixo, torre, container, acessos e vista da obra no bairro. Priorize: escala/encaixe, fidelidade ao lugar, cobertura verdadeira, rotas, leitura de inimigos e custo. Confira especialmente: Terreno ondulado, escavações, subidas, container/bunker e rotas. Não transformar andaime decorativo em promessa de caminho inexistente. Registre até cinco problemas concretos com imagem/câmera e arquivo:linha, separando regressão de aprofundamento ainda ausente. Não aceite pelo número de testes ou assets. Não edite simultaneamente com o builder; devolva correções delimitadas ou assuma o mapa somente após handoff explícito. Se precisar de Mint, confira o inventário antes para não duplicar geração. Declare o que não foi demonstrado.
```
