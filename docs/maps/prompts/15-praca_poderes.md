# Praça dos Três Poderes — prompt de execução

Copie o bloco abaixo para GLM; ele também funciona no outro modelo se houver um único responsável pelo mapa.

```text
Você é o responsável por aprofundar visualmente Praça dos Três Poderes no CSBrasil, usando pesquisa real, acervo existente e Mint.gg quando necessário. Entregue implementação e evidência, não apenas planejamento.

CONTEXTO E FONTE
Lane claude/praca-poderes-visual já existe; conferir HEAD/ledger/PR antes de iniciar. Props carregam internamente, props vazio no registro não prova ausência.
...
ISOLAMENTO
Um builder por mapa. Não edite primary, checkout alheio nem a worktree integradora em paralelo. Parta do checkpoint integrado citado no índice e use worktree própria para este mapa; se já houver dono ativo, coordene a posse antes de editar. Seu escopo é public/js/map_brasilia.js, helpers exclusivos, assets exclusivos e gates/docs deste mapa. maps.js, game.js, main.js, look.js, mapprops.js, package.json e registros compartilhados exigem integração sequencial: entregue patch separado ao integrador. Não altere core, armas, HUD ou áudio como efeito colateral.

DIREÇÃO VISUAL
Brasília brasileira reconhecível, com escala e composição dos marcos, esplanada, pavimentação e mobiliário cívico. Melhorar material e profundidade sem distorcer os landmarks.
...
ACERVO E MINT.GG
Inventariar GLBs internos antes de Mint. Produzir apenas mobiliário ou módulo de fachada faltante; não substituir monumentos por versões genéricas.
...
Silhuetas dos marcos, pontos de conflito, coberturas e spawn. Não encher a esplanada de adereços para disfarçar ausência de escala.
...
VERIFICAÇÃO
Use Node compatível com o projeto; nesta máquina PATH=/opt/homebrew/bin:$PATH. Confirme scripts disponíveis antes de executar: eval:mapcontrato; eval:spawn; eval:shaderbudget; gates próprios disponíveis. Acrescente contratos/spawns/CTF pertinentes. Um único browser pesado por vez no conjunto das frentes: reserve sua vez com o integrador. Capturas reais 1200×800, GLBs de fato carregados, ângulos repetíveis: eixo monumental, cada landmark, travessias da praça e visada desde ambos spawns.
...
```text
Revise Praça dos Três Poderes independentemente. Primeiro abra as capturas reais antes/depois da execução acima; só depois leia o builder. Não receba a justificativa do autor como prova. Compare a direção visual descrita neste arquivo com referências reais, confirme GLBs carregados e examine eixo monumental, cada landmark, travessias da praça e visada desde ambos spawns. Priorize: escala/encaixe, fidelidade ao lugar, cobertura verdadeira, rotas, leitura de inimigos e custo. Confira especialmente: Silhuetas dos marcos, pontos de conflito, coberturas e spawn. Não encher a esplanada de adereços para disfarçar ausência de escala. Registre até cinco problemas concretos com imagem/câmera e arquivo:linha, separando regressão de aprofundamento ainda ausente. Não aceite pelo número de testes ou assets. Não edite simultaneamente com o builder; devolva correções delimitadas ou assuma o mapa somente após handoff explícito. Se precisar de Mint, confira o inventário antes para não duplicar geração. Declare o que não foi demonstrado.
```
