# UPA 24h da Treta — prompt de execução

Copie o bloco abaixo para Claude; ele também funciona no outro modelo se houver um único responsável pelo mapa.

```text
Você é o responsável por aprofundar visualmente UPA 24h da Treta no CSBrasil, usando pesquisa real, acervo existente e Mint.gg quando necessário. Entregue implementação e evidência, não apenas planejamento.

CONTEXTO E FONTE
PR #337 integrado; diagnóstico legado #538. O builder já possui macas, monitores, respiradores e cadeiras: não afirmar ausência sem olhar.
...
ISOLAMENTO
Um builder por mapa. Não edite primary, checkout alheio nem a worktree integradora em paralelo. Parta do checkpoint integrado citado no índice e use worktree própria para este mapa; se já houver dono ativo, coordene a posse antes de editar. Seu escopo é public/js/map_upa.js, helpers exclusivos, assets exclusivos e gates/docs deste mapa. maps.js, game.js, main.js, look.js, mapprops.js, package.json e registros compartilhados exigem integração sequencial: entregue patch separado ao integrador. Não altere core, armas, HUD ou áudio como efeito colateral.

DIREÇÃO VISUAL
UPA brasileira reconhecível: recepção, triagem, consultórios, observação e emergência com sinalização, materiais laváveis e mobiliário de saúde. Eliminar leitura comercial dos manequins/gôndolas reaproveitados.
...
ACERVO E MINT.GG
Inventariar kit hospitalar procedural atual. Mint para cadeira de espera, maca, armário clínico, divisória e carrinho de emergência somente quando a versão atual for insuficiente; evitar hospital inteiro em um GLB.
...
Volumes de cobertura, portas, corredor central e visibilidade em interior. Substituição visual mantém espaço físico e arsenal.
...
```text
Revise UPA 24h da Treta independentemente. Primeiro abra as capturas reais antes/depois da execução acima; só depois leia o builder. Não receba a justificativa do autor como prova. Compare a direção visual descrita neste arquivo com referências reais, confirme GLBs carregados e examine recepção, corredor, consultório, emergência, farmácia e confronto entre dois cômodos. Priorize: escala/encaixe, fidelidade ao lugar, cobertura verdadeira, rotas, leitura de inimigos e custo. Confira especialmente: Volumes de cobertura, portas, corredor central e visibilidade em interior. Substituição visual mantém espaço físico e arsenal. Registre até cinco problemas concretos com imagem/câmera e arquivo:linha, separando regressão de aprofundamento ainda ausente. Não aceite pelo número de testes ou assets. Não edite simultaneamente com o builder; devolva correções delimitadas ou assuma o mapa somente após handoff explícito. Se precisar de Mint, confira o inventário antes para não duplicar geração. Declare o que não foi demonstrado.
```
