...
## R5 — casa frontal com duas entradas e janelas cruzadas
- `origin/main` alpha.233 foi integrado no merge `0099438e`; conflitos ficaram restritos aos documentos gerados e ao registro de bugs, preservados por regeneração e união dos relatos. Nenhuma modificação de outro checkout.
- A casa frontal agora tem duas entradas reais: escada externa para quem vem da rua/respawn e passarela do patamar 1 para quem vem de cima. A passarela entra por um portal no prédio da laje e pela porta lateral da casa; os dois vãos são geometria e colisão reais, não atravessamento de parede.
- A janela frontal mantém tiro ao patamar e a traseira abre tiro para a rua. Peitoris e vergas continuam sólidos. `escadao-home-check` percorre ida/volta pela entrada inferior e superior, verifica as duas linhas de tiro, piso único e caminho de bots; a mutação `porta-fechada` volta a falhar.
- Regressões após a alteração: grafo 609/609 com oito rotas para Deagle; passagens inferiores 1.115 posições; contornos 740 posições; horizonte 3.840 triângulos/64 casas sem colisão; contratos Escadão e mapa global verdes. `ARCH1`, `DOCS1` e build alpha.233 passam (aviso herdado de Node 23→runtime Vercel 24). Próximo autorizado: PR, CI e merge se os checks remotos passarem.
- PR522 inicialmente reprovou `escadao-structure`: a guarda externa da primeira versão da passarela ficava a menos de um raio corporal do lance 1. A guarda foi deslocada para x=0,5, a passagem útil preservada e a suíte remota equivalente voltou verde: graph/home/structure/descent/details 5/5, incluindo a mutação porta-fechada. Build repetido verde. Aguardar nova CI antes do merge.
...
## R5 — fechamento revisável sobre a main (08/09/2026)
- Retomada em `codex/escadao-r5-fechamento`, criada diretamente de `origin/main` `e67addf4` (alpha.242). Os checkpoints locais `a733dd1b`, `aa5ea7f4` e `27791c99` já estavam integrados pela PR #522; a PR #529 ampliou o contrato da casa central e das casas laterais. Não se reaplicou geometria já mesclada.
- A limpeza removeu somente três saídas geradas e rastreadas de `tools/eval` (`map_check.json`, `mat_check.json`, `mat_scenes.json`). Nenhum fonte, asset ou trabalho legítimo foi descartado.
- Casa central: baseline dedicado 24/24. As duas janelas opostas pertencem ao mesmo cômodo, aceitam cápsula de olho/alvo e tiro/revide; piso contínuo em 60 células; acessos superior e inferior concluem em 102/217 frames. Cinco mutantes (`janela-fechada`, `janela-oposta-fechada`, `piso-reaberto`, `acesso-removido`, `casa-mirante-fechada`) mordem as cláusulas esperadas.
- Casas superiores: rotas laterais oeste/leste chegam ao interior em 66/69 frames no gate Node. A prova browser adicional percorre ida e volta com `_updatePlayer` em 108 frames por casa e registra as portas pelo exterior em `artifacts/escadao-visual/r5-closeout/browser-r5-proof/`.
- Passagem inferior: a prova browser percorre ida e volta sob o patamar oeste em 237 frames, sem teleporte; a captura `sob-patamar.png` mostra a folga. A suíte de runtime completa passa 140/140 séries, 12/12 escadas, zero amostra sem piso e zero head hit. Os mutantes de varal, escada bloqueada e abrigo removido são detectados nas cláusulas EV1, EV2 e EV7.
- Navegação e escala de partida: grafo 669/669, dez destinos de escada, zero entre 979 posições altas com LOS para spawn; 5x5 carrega nove bots e 8x8 quinze, todos finitos e em movimento no ensaio browser. Horizonte: 3.840 triângulos e 64 casas, sem collider/occluder.
- Evidência visual 3:2: 21 capturas do jogo real em `artifacts/escadao-visual/r5-closeout/browser-3x2/`. Crítica independente confirmou as duas janelas, o piso e o horizonte, e pediu provas externas/colisão do mirante e passagem. A revisão de retorno aprovou as novas portas externas e as três rotas de `browser-r5-proof`, sem bloqueador funcional restante; registrou como observações a janela oeste estreita e o corredor inferior escuro. Aprovação humana final continua separada dos gates técnicos.
- Correção desta branch: `escadao-visual-check.mjs` voltou a derivar o raio CTF da declaração atual de `point` em `_initCTF`. A main havia trocado `return { ... }` por `const point = { ... }; return point`, deixando o gate vermelho antes de abrir o navegador. Após a correção, baseline EV0–EV7 e os três mutantes passam sem duplicar a constante física.
- Validação concluída: gates dedicados de casa, casa central, mirante, estrutura, descida, detalhes, grafo, rota/LOS, contrato, fachada, passagens, contorno e horizonte; build, `DOCS1` e `ARCH1`; browser 3:2 e 5x5/8x8. Servidor local exclusivo mantido em `http://127.0.0.1:8158/?debug=1&map=escadao&auto=B%2Csertanejo&lang=pt&local=r5-closeout`.
- Próximo: fazer checkpoint DCO+Agent, publicar branch e abrir PR draft contra `main`; aguardar revisão/CI e avaliação humana. Sem merge ou deploy nesta rodada.
