...
**14,17 m** em 145 nós. Teto adotado: **10,50 m**. Estado entregue nesta revisão: **9,69 m** em 172 nós.
...
| --- | --- | --- |
| Fileiras de rack | 6 | 48 blocos de 2,3 m × 3 m; um `estante_pallets.glb` hero por cabeceira e módulos procedurais no miolo |
...
Som: a alpha.246 não publica os loops de ventilação/cidade usados no ramo histórico.
O mapa fica deliberadamente em silêncio, sem fallback genérico, até existir soundscape
aprovado e alcançável. A fauna visual permanece ativa.
...
Os GLBs repetidos de rack, carros, ilhas, freezers e balcões usam `PropBatch`. O rack
detalhado funciona como hero asset nas seis cabeceiras; o miolo mantém carga, altura,
colisor e silhueta procedurais. No Chrome/Metal real, build estático:

| Matriz | p95 main | p95 candidata | Draws main → candidata | Triângulos main → candidata |
| --- | ---: | ---: | ---: | ---: |
| 5x5, 1536×1024, médio | 9,8 ms | 10,4 ms | 1.420 → 1.239 | 1.553.707 → 2.085.638 |
| 8x8, 1600×900, baixo | 9,9 ms | 9,8 ms | 830 → 868 | 780.552 → 1.099.673 |

As duas matrizes ficaram dentro do teto de +10% de p95 e sem frames acima de 100 ms.
Os recibos e 24 capturas A/B ficam em `artifacts/atacadao/browser/` (ignorado pelo Git).
