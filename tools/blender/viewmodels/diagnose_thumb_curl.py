"""Render measured support-thumb curl variants at the AK foregrip contact."""
...
OUT = ROOT / "artifacts" / "viewmodels" / "ak-pilot" / "renders" / "thumb_curl_grid"
...
for axis_name, axis in axes.items():
    for sign in (-1.0, 1.0):
        builder.pose_idle(
            ...
            left_local=(0.245, 0.002, -0.080),
            left_close=0.0,
        )
        # Keep the measured non-thumb grip fixed while only the thumb changes.
        ...
        scene.render.filepath = str(OUT / f"thumb_{axis_name}_{label}.png")
        bpy.ops.render.render(write_still=True)

print(f"THUMB_CURL_GRID {OUT}")
