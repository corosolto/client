...
import { launchLajesMatch } from './lajes-browser-launch.mjs';
import { mkdirSync, writeFileSync } from 'node:fs';
const option = (name, fallback) => process.argv.find(a => a.startsWith(`--${name}=`))?.split('=').slice(1).join('=') || fallback;
const out=option('out','artifacts/lajes-visual/browser');const base=option('base','http://127.0.0.1:8147');const photos=Number(option('fotos','9'));if(!Number.isInteger(photos)||photos<0||photos>16)throw Error('--fotos deve ser 0–16');mkdirSync(out,{recursive:true});
const quality=option('quality','med');if(!['low','med','high'].includes(quality))throw Error('quality inválida');
const width=Number(option('width','1536')),height=Number(option('height','1024'));if(!Number.isInteger(width)||!Number.isInteger(height)||width<800||height<600)throw Error('viewport inválido');
const teams=Number(option('teams','4')),mode=option('mode','ctf'),allowInherited=process.argv.includes('--allow-inherited');if(![4,5,8].includes(teams)||!['dm','ctf'].includes(mode))throw Error('partida inválida');
...
await page.addInitScript(({quality,teams})=>localStorage.setItem('awpbr_settings',JSON.stringify({quality,bots:teams,vol:0,speech:false})),{quality,teams});
await launchLajesMatch(page,{base,mode});
await page.waitForTimeout(2000);
console.log('game live');if(!glbs.length)throw Error('Sem GLBs HTTP200: não sei medir visual real');
const info=await page.evaluate(()=>{const g=window.__game;if(g._mapId!=='lajes')throw Error('wrong map');g.paused=true;g.player.hp=1e9;for(const b of g.bots){b.hp=1e9;b.mesh.group.visible=false;}return {initialSpawn:g.player.pos.toArray(),map:g._mapId,bots:g.bots.length,ctf:g.ctf,ctfPoints:g.ctfPts?.length||0,keys:Object.keys(g).filter(k=>/render|camera|scene/.test(k)),worldKeys:Object.keys(g.world),settings:g.settings,fov:g.camera.fov,render:{memory:g.renderer.info.memory,programs:g.renderer.info.programs.length},renderer:g.renderer?.__csWebgl};});
...
const debt=classifyLajesBrowserDebt(errors,failed);writeFileSync(`${out}/boot.json`,JSON.stringify({info,errors,failed,glbs,debt},null,2));
if(info.bots!==teams*2-1||info.ctf!==(mode==='ctf')||(mode==='ctf'&&info.ctfPoints!==4)
  ||info.renderer?.api!=='webgl2'||info.renderer?.software===true
  ||debt.unexpected.length||(!allowInherited&&debt.known.length))process.exitCode=1;
const poses=[['spawn-norte',-3,0,-27,-Math.PI/2,0],['spawn-sul',3,0,27,Math.PI/2,0],['praca-sul',0,0,-6,Math.PI,0],['praca-norte',0,0,5,0,0],['fachada-oeste',-1.7,0,-4.5,Math.PI/2,0],['beco-oeste',-13.8,0,-16,Math.PI,-.02],['escada-norte-oeste',-9,0,-26.5,Math.PI,.1],['laje-oeste',-9,3.1,-12,Math.PI,-.1],['praca-da-laje',-6.3,3.1,-4,-Math.PI/2,-.25],['rua-central',0,0,-18,Math.PI,0],['beco-leste',13.8,0,16,0,0],['travessa-sob-ponte',-11,0,0,-Math.PI/2,0],['esquina-respawn',-13.8,0,-27,-Math.PI/2,0],['escada-norte-leste',9,0,-26.5,Math.PI,.1],['escada-sul-oeste',-9,0,26.5,0,.1],['escada-sul-leste',9,0,26.5,0,.1]];
...
writeFileSync(`${out}/cameras.json`,JSON.stringify({viewport:[width,height],fov:info.fov,quality:info.settings?.quality,teams,mode,poses},null,2));
