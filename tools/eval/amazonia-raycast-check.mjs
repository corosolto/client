// PR #527: compara consultas reais com o raycast linear e conta triângulos enviados ao Three.
...
// Redução de uma ordem no trabalho da etapa que tomou 93% da CPU: relatório do PR #527.
const g = bootGame('amazonia', { textures: initTextures(), bots: 8, seed: 4321 });
const meshes = g.world.occluders.filter(m => ['madeira-amazonia', 'chao-de-mata'].includes(m.name) && !m.isInstancedMesh);
...
THREE.Mesh.prototype._computeIntersections = function(...args) {
  if (['madeira-amazonia', 'chao-de-mata'].includes(this.name)) {
...
try {
  ...
  const valid = meshes.length > 0 && rays.length > 0 && hitRays > 0 && baseline.tested > 0 && mismatches === 0 && ratio <= .1 && occlusionMismatches === 0 && occlusionCalls === rays.length + 1 && intersections > 0 && afterHit === 0 && stress.valid && smokeBlocks;
  ...
  console.log(`${valid ? '✓' : '✗'} AMRP1 raycast Amazônia: ${JSON.stringify(report)}`);
