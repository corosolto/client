...
try {
  ...
  const extraQuery = query ? `&${query.replace(/^\?/, '')}` : '';
  await page.goto(`${base}/?debug=1&auto=P,mst&map=amazonia&perfilauto=0${extraQuery}`, {
  ...
  const result = await page.evaluate(() => {
    ...
    const result = {
      ...
      renderProfile: game.world.amazonia?.renderProfile || null,
    };
    game.paused = true;
  ...
  const suffix = query ? `-${query.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}` : '';
  const label = `amazonia-${teamSize}-${quality}-${width}x${height}${suffix}`;
