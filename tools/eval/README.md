...
| `relevo-check.mjs` | Cota do chão na área ANDÁVEL (alagamento pelo `_freeSpot` real): amplitude, fração plana, declive, subida contínua a pé e silhueta. Só o `campomorro` tem contrato; os demais saem informativos. Mutantes: `terreno-plano`, `silhueta-rasa`, `degrau-unico`. |
...
| `ambience-check.mjs` | Abre os GLBs reais de rato/pombo em `lajes`, `corrego` e `escadao`; mede determinismo, reação a tiro pelo `Game`, degradação LOWQ, custo e largura/duração do traçante. `--fotos` captura 3:2; mutantes por cláusula. |
