...
const expected = { ilustracao: ['PV1'], autoplay: ['PV2'], congelado: ['PV3'], 'sem-saida': ['PV4'], 'sem-pausa': ['PV4', 'PV5'], 'movimento-reduzido': ['PV5'], 'astro-cru': ['PV6'], 'video-fora-do-quadro': ['PV7'] };
...
try {
  ...
  if (['sem-saida', 'video-fora-do-quadro'].includes(mut)) await page.route('**/js/sertao_map_preview.js*', async route => {
    const response = await route.fetch(), source = await response.text();
    const body = mut === 'sem-saida' ? source.replace("host.addEventListener('pointerleave', stop);", '') : source.replace('top: `${img.offsetTop}px`', 'top: `${img.offsetTop + 20}px`');
  ...
  const alignment = await card.evaluate(c => {
    const img = c.querySelector('img').getBoundingClientRect(), v = c.querySelector('video')?.getBoundingClientRect();
    return v ? ['x', 'y', 'width', 'height'].map(k => Math.abs(img[k] - v[k])) : null;
  });
  check('PV7', alignment?.every(delta => delta < 1), alignment);
