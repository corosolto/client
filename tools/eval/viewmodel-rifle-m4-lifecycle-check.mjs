...
function matrix({ decide = viewmodelVisibility, accepts = acceptsAuthoredLoad,
  ...
  for (let cycle = 0; cycle < 30; cycle += 1) {
    ...
    if (accepts({ request, activeRequest, key: 'ar#m4', activeKey: 'ar#m4', utility: false })) {
    ...
    if (!accepts({ request: activeRequest, activeRequest, key: 'ar#m4', activeKey: 'ar#m4', utility: false })) {
...
check(VM_WEAPON.m4.baked === true, 'M4 usa candidato baked por arma');
const baseline = matrix();
check(baseline.errors.length === 0, 'M4 atravessa lifecycle 30x em 3:2 e 16:9', `${baseline.samples} amostras`);
