...
try {
  ...
  await page.goto('http://127.0.0.1:8146/?debug=1&map=amazonia&auto=B,sertanejo&perfilauto=0', { waitUntil: 'domcontentloaded', timeout: 120000 });
  ...
  const trees = await page.evaluate(async () => {
    ...
    g.world.root.traverse(o => {
      ...
      for (let i = 0; i < o.count; i++) {
        ...
        if (extent > .45 + 1e-4) offenders.push({ x:a.x,z:a.z,extent });
      }
    });
  ...
  writeFileSync(`${out}/capture.json`, JSON.stringify({ setup, cameras: metrics, runtime, errors, assets, performanceApproval: 'PENDING: no exclusive GPU window' }, null, 2));
