...
/* ============================================================================
   ...
   ── O QUE ELA MEDE (mundo construído pelo harness, node puro) ──────────────
     ...
     OBRAS7 · TRÊS DECISÕES — cada spawn alcança o objetivo oposto por oeste,
              centro e leste. Os caminhos passam por portais separados e não
              colapsam no mesmo corredor depois de sair do spawn.
...
/* ================= OBRAS7 · TRÊS DECISÕES POR SPAWN ================= */
const routeFor = (team, side, laneX) => {
  const spawn = world.spawns[team][0];
  const gate = { x: laneX, z: side * 9 };
  const target = { x: -10, z: side * -14 };
  const startIdx = world.nearestWaypoint(spawn.x, spawn.z);
  const gateIdx = world.nearestWaypoint(gate.x, gate.z);
  const targetIdx = world.nearestWaypoint(target.x, target.z);
  const first = world.findPath(startIdx, gateIdx);
  const second = world.findPath(gateIdx, targetIdx);
  const gateNode = world.waypoints.nodes[gateIdx];
  const reaches = first.length > 1 && first.at(-1) === gateIdx
    && second.length > 1 && second.at(-1) === targetIdx
    && Math.hypot(gateNode.x - gate.x, gateNode.z - gate.z) <= 3;
  return { reaches, ids: [...first, ...second.slice(1)], gateNode };
};
const routeSets = [];
let routeEvidence = [];
for (const [team, side] of [['E', -1], ['B', 1]]) {
  const routes = [-20, 0, 20].map((x) => routeFor(team, side, x));
  const jaccards = [];
  for (let i = 0; i < routes.length; i++) for (let j = i + 1; j < routes.length; j++) {
    const a = new Set(routes[i].ids.slice(2, -2));
    const b = new Set(routes[j].ids.slice(2, -2));
    const intersection = [...a].filter((id) => b.has(id)).length;
    const union = new Set([...a, ...b]).size;
    jaccards.push(union ? intersection / union : 1);
  }
  routeSets.push(routes.every((r) => r.reaches) && Math.max(...jaccards) <= 0.65);
  routeEvidence.push(`${team} ${routes.map((r) => r.ids.length).join('/')} nós, overlap ${(Math.max(...jaccards) * 100).toFixed(0)}%`);
}
