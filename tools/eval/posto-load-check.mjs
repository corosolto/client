/* Posto: carga CTF determinística em 5x5 e 8x8, usando Game e bots de produção. */
...
console.log(`POSTO CARGA: ${results.map((r, i) => `${sizes[i]}x${sizes[i]} bots=${r.bots} stuck=${r.stuckPct}% eff=${r.eff} spinRoam=${r.spinRoam} laneSpread=${r.laneSpread}`).join(' · ')}`);
