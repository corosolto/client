...
function matrix({ decide = viewmodelVisibility, accepts = acceptsAuthoredLoad,
  ...
  for (let cycle = 0; cycle < 30; cycle += 1) {
    ...
    if (accepts({ request, activeRequest, key: 'ar#md97', activeKey: 'ar#md97', utility: false })) {
    ...
    if (!accepts({ request: activeRequest, activeRequest, key: 'ar#md97', activeKey: 'ar#md97', utility: false })) {
...
check(VM_WEAPON.md97.baked === true, 'MD97 usa candidato baked por arma');
const baseline = matrix();
check(baseline.errors.length === 0, 'MD97 atravessa lifecycle 30x em 3:2 e 16:9', `${baseline.samples} amostras`);
