...
function matrix({ decide = viewmodelVisibility, accepts = acceptsAuthoredLoad,
  ...
  for (let cycle = 0; cycle < 30; cycle += 1) {
    ...
    if (accepts({ request, activeRequest, key: 'ak#akm', activeKey: 'ak#akm', utility: false })) {
    ...
    if (!accepts({ request: activeRequest, activeRequest, key: 'ak#akm', activeKey: 'ak#akm', utility: false })) {
...
check(VM_FAMILY.ak.ready === false, 'família AK continua ready:false');
check(VM_WEAPON.akm.baked === true, 'AKM usa candidato baked por arma');
const baseline = matrix();
check(baseline.errors.length === 0, 'AKM atravessa lifecycle 30x em 3:2 e 16:9', `${baseline.samples} amostras`);
