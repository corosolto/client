...
function matrix({ decide = viewmodelVisibility, accepts = acceptsAuthoredLoad,
  ...
  for (let cycle = 0; cycle < 30; cycle += 1) {
    ...
    if (accepts({ request, activeRequest, key: 'sniper#m400', activeKey: 'sniper#m400', utility: false })) {
    ...
    if (!accepts({ request: activeRequest, activeRequest, key: 'sniper#m400', activeKey: 'sniper#m400', utility: false })) {
...
check(VM_FAMILY.sniper.ready === false, 'família sniper continua ready:false');
check(VM_WEAPON.m400.baked === true, 'M400 usa candidato baked por arma');
const baseline = matrix();
check(baseline.errors.length === 0, 'M400 atravessa lifecycle 30x em 3:2 e 16:9', `${baseline.samples} amostras`);
