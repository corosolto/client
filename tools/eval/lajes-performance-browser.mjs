...
const width=Number(option('width','1536')),height=Number(option('height','1024'));
const allowInherited=process.argv.includes('--allow-inherited');
if (!maps.every(m => ['lajes', 'piscina_treta'].includes(m)) || !teams.every(n => [5,8].includes(n)) || !['low','med','high'].includes(quality) || !(seconds >= 5 && seconds <= 180) || !Number.isInteger(width) || !Number.isInteger(height) || width<800 || height<600) throw Error('Argumentos inválidos');
...
try {
  for (const map of maps) for (const bots of teams) {
    ...
    if (map === 'lajes') await launchLajesMatch(page, { base, mode: 'dm' });
    else {
    ...
    const result = await page.evaluate(() => {
      ...
      const result = { elapsed: performance.now()-m.start, frames: frames.length, renderedFrames: (g._rafFrames || 0)-m.renderedStart,
        ...
        actualBots: g.bots.length, quality: g.settings.quality, viewport: [innerWidth,innerHeight], gpu: g.renderer.__csWebgl, state: g.state, simulationTime: g.time };
      g.paused = true; return result;
    });
    const name = `${map}-${bots}-${quality}${linear ? '-linear' : ''}`;
    result.errors = errors; result.failed = failed; result.debt = classifyLajesBrowserDebt(errors,failed); result.linear = linear;
    writeFileSync(`${out}/${name}.json`, JSON.stringify(result, null, 2));
    await page.screenshot({ path: `${out}/${name}.png` });
    console.log(JSON.stringify({ name, p50: result.p50, p95: result.p95, max: result.max, over100ms: result.over100ms, debt:result.debt }));
    if (result.debt.unexpected.length || (!allowInherited&&result.debt.known.length)
      || result.frames === 0 || result.actualBots !== bots*2-1
      || result.gpu?.api !== 'webgl2' || result.gpu?.software === true) process.exitCode = 1;
    await context.close();
  }
