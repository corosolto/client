...
/* A lista não pode crescer: script novo nasce com fixture, e não com dispensa. */
const naBase = git(['show', `${base}:tools/eval/fixture-check.mjs`]);
const daBase = [...naBase.matchAll(/'(scripts\/ci\/[^']+)'/g)].map((m) => m[1]);
if (daBase.length && SEM_FIXTURE.size > daBase.length) {
  falhas.push(`FX3 a lista SEM_FIXTURE cresceu (${daBase.length} → ${SEM_FIXTURE.size}); ela só encolhe`);
}
