...
async function preload(mutant = '') {
  ...
  if (mutant === 'preload-global') source = source.replace("Object.keys(ASSETS).filter(id => !['calango', 'lagarto'].includes(id))", 'Object.keys(ASSETS)');
  ...
  await api.preloadAmbientLife(['calango', 'lagarto']);
  return [!defaultRequests.some(url => /calango|lagarto/.test(url)) && defaultRequests.some(url => /galinha_campo/.test(url)),
    requests.length === 2 && requests.some(url => /calango_quadrupede/.test(url)) && requests.some(url => /lagarto_sertao/.test(url))];
}
...
checks.push({ id: 'SI1', ok: base[0], meaning: 'Fauna específica não amplia download dos outros mapas' },
  { id: 'SI2', ok: base[1], meaning: 'Lista explícita realmente carrega ambos os modelos do Sertão' },
