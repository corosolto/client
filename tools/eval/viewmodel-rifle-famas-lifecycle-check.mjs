...
function matrix({ decide = viewmodelVisibility, accepts = acceptsAuthoredLoad,
  ...
  for (let cycle = 0; cycle < 30; cycle += 1) {
    ...
    if (accepts({ request, activeRequest, key: 'ar#famas', activeKey: 'ar#famas', utility: false })) {
    ...
    if (!accepts({ request: activeRequest, activeRequest, key: 'ar#famas', activeKey: 'ar#famas', utility: false })) {
...
check(VM_WEAPON.famas.baked === true, 'FAMAS usa candidato baked por arma');
const baseline = matrix();
check(baseline.errors.length === 0, 'FAMAS atravessa lifecycle 30x em 3:2 e 16:9', `${baseline.samples} amostras`);
