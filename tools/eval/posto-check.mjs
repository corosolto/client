...
/* ============================================================================
   ...
   ── O QUE ELA MEDE (mundo construído pelo harness, node puro) ──────────────
     ...
     POSTO2 · LOJA INTERIOR — sala com teto, ≥ 4 gôndolas que COLIDEM dentro dela,
              ...
              rotas dos dois times às duas entradas e ≥ 2 pickups dentro da sala.
   ...
   ── AS MUTAÇÕES QUE PROVAM ─────────────────────────────────────────────────
     ...
     --mutante=rota-loja ...... desconecta a sala da malha       -> POSTO2
...
const MUTANTES = new Set(['bomba-caixa', 'loja-macica', 'rota-loja', 'sem-cobertura', 'patio-aberto', 'patio-limpo', 'radio-limpo']);
...
if (MUT === 'rota-loja') {
  const nos = world.waypoints.nodes, adj = world.waypoints.adj;
  for (let i = 0; i < adj.length; i++) adj[i] = adj[i].filter((j) => dentroSala(nos[i].x, nos[i].z) === dentroSala(nos[j].x, nos[j].z));
}
...
const alvosSala = [-3.4, 3.4].map((z) => world.nearestWaypoint(-23, z));
const rotasSala = ['E', 'B'].flatMap((team) => alvosSala.map((alvo) => {
  const spawn = world.spawns[team][0];
  return world.findPath(world.nearestWaypoint(spawn.x, spawn.z), alvo);
}));
const rotasValidas = rotasSala.filter((rota) => rota.length >= 2 && dentroSala(
  world.waypoints.nodes[rota.at(-1)]?.x, world.waypoints.nodes[rota.at(-1)]?.z));
const lootSala = world.pickups.filter((p) => dentroSala(p.x, p.z));
const posto2 = gondolasComColisor.length >= 4
  && aberturas.length >= 2
  && !!teto && teto.position.y >= 3.4
  && nosSala.length >= 6
  && rotasValidas.length === 4
  && lootSala.length >= 2;
...
const r = [
  ...
  linha('POSTO2', posto2, `${gondolasComColisor.length} gôndolas com colisor · ${aberturas.length} aberturas · teto y=${teto ? teto.position.y.toFixed(2) : '—'} · ${nosSala.length} nós · ${rotasValidas.length}/4 rotas E/B→duas entradas · ${lootSala.length} pickups`),
