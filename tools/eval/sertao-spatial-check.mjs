...
const MUTANTS = {
  ...
  'casa-tag': { target: 'SP3', apply(w) { const c = w.colliders.find(c => c.tag?.startsWith('sertao-casa-')
    && w.colliders.filter(d => d.tag === c.tag).length === 1); if (!c) throw Error('tag única ausente'); c.tag = 'mutante-sem-casa'; } },
