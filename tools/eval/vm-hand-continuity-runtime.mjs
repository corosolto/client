...
for (const k of args.keys()) if (!['saida','porta','sweep'].includes(k)) throw Error(`flag ${k}`);
...
try {
  ...
  const capture=async(label)=>{
    await page.waitForFunction(()=>{
      ...
      return meshes.every(o=>(Array.isArray(o.material)?o.material:[o.material]).every(m=>!m.userData.teamHands||m.map?.image?.width>0));
    });
    const state=await page.evaluate(()=>{
      ...
      scene.traverse(o=>{if(o.isMesh)for(const m of (Array.isArray(o.material)?o.material:[o.material])){
        if(m.userData.teamHands)materials.push({mesh:o.name,...m.userData.teamHands,map:m.map.image.currentSrc||m.map.image.src,width:m.map.image.width,color:m.color.getHexString()});
      }});
    ...
    report.states.push({label,file,...state});
