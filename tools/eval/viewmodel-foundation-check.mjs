...
// #618: a malha própria (cache ou `weaponModel`) é resolvida e, se faltar, o pack fica.
const wrapAt = vmweapon.indexOf('if (!jaCacheado && !recemCriado) { fallbackParaOPack(entry);');
const hideAt = vmweapon.indexOf('hidePackGun(entry)', vmweapon.indexOf('export function attachMintWeapon'));
...
check(![...tracked].some((file) => /models\/viewmodels\/.*(?:mosin|svd|sks)/i.test(file)
  && !/^public\/models\/viewmodels\/coro\/(?:mosin|svd|sks)-hires\.glb$/.test(file)),
'bytes privados de Mosin/SVD/SKS não foram promovidos ao Git');
