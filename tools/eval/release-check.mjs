 ...
 *
 * Mutações: --mutante=nome-antigo | semcreditos | semdco | semdocs | semrollback.
 */
...
if (mutante) {
  const antes = JSON.stringify([comandos, commitsRelease, docsRelease, rollbackRelease]);
  ...
  if (JSON.stringify([comandos, commitsRelease, docsRelease, rollbackRelease]) === antes) throw new Error(`MUTANTE NAO APLICOU: ${mutante}`);
}
...
const ok = total > 0 && nomes === total && creditos === total && totalCommits > 0 && dco === totalCommits && docsOk === docsRelease.length && rollbackRelease;
...
if (!ok) {
  console.error('Release inválido: preserve nome/créditos, DCO e o build versionado de public/docs.');
  process.exit(1);
}
