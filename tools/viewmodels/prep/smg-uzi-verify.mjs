#!/usr/bin/env node
// Gate causal da UZI final. Antes da receita, o manifesto e o produto não existem.
...
const cfg = manifest.candidates?.uzi;
check(Boolean(cfg), 'entrada UZI ausente do manifesto');
const file = cfg ? path.join(ASSET_ROOT, cfg.file) : '';
check(Boolean(file) && fs.existsSync(file), `produto UZI ausente em ${file || '<sem rota>'}`);
if (!file || !fs.existsSync(file)) {
  console.log(`VM_SMG_UZI=${JSON.stringify({ ok: false, file, failures })}`);
  process.exit(1);
}
...
check(cfg.ready === false, 'UZI precisa permanecer ready:false');
...
check(Boolean(gun?.isMesh), 'malha UZI própria não preservada');
check(Boolean(mint && muzzle && sight), 'marcador baked e sockets ADS/muzzle ausentes');
check(Boolean(arms && weapon && mag && bolt && trigger), 'rig/mecanismos próprios incompletos');
...
check(trackMotion(gltf, 'shoot', 'MINT_MECH_UZI_BOLT.position') >= 0.04, 'shoot não cicla o ferrolho próprio');
check(trackMotion(gltf, 'shoot', 'MINT_MECH_UZI_TRIGGER.quaternion') >= 0.08, 'shoot não aciona o gatilho próprio');
...
check(trackMotion(gltf, 'reload_empty', 'MINT_MECH_UZI_BOLT.position') >= 0.04, 'reload_empty não aciona o ferrolho/alavanca');
...
await mutant('sem-arma', (copy) => copy.scene.getObjectByName('GEO_WEAPON_UZI')?.removeFromParent(), (copy) => !copy.scene.getObjectByName('GEO_WEAPON_UZI'));
await mutant('sem-pente', (copy) => copy.scene.getObjectByName('MINT_WEAPON_MAG_UZI')?.removeFromParent(), (copy) => !copy.scene.getObjectByName('MINT_WEAPON_MAG_UZI'));
await mutant('sem-marker', (copy) => copy.scene.getObjectByName('MINT_WEAPON_UZI')?.removeFromParent(), (copy) => !copy.scene.getObjectByName('MINT_WEAPON_UZI'));
