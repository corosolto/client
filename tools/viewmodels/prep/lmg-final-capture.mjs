#!/usr/bin/env node
// Evidência no jogo real para o candidato lmg. O diretório padrão é privado e
...
const OUT = path.resolve(process.env.CSBRASIL_VM_EVIDENCE_DIR
  || '/Users/ruben/csbrasil-private-assets/generated/viewmodels-catalog-final/evidence/lmg-20260914');
const errors = [];
const records = [];
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmready=lmg&vmweapon=lmg&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('lmg'))) throw new Error(`${aspect}: lmg não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('lmg')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `lmg-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('lmg');
  ...
  const pose = async (clipName, fraction) => page.evaluate(({ clipName, fraction }) => {
    ...
    const entry = vm.entry('lmg');
    if (vm.__lmgQaUpdate) vm.update = vm.__lmgQaUpdate;
    vm.setAim('lmg', 0);
    ...
    if (!vm.__lmgQaUpdate) vm.__lmgQaUpdate = vm.update;
    vm.update = () => {};
  }, { clipName, fraction });
  const drawAt = async (fraction) => page.evaluate((fraction) => {
    const vm = window.__authoredVm;
    if (!vm.__lmgQaUpdate) vm.__lmgQaUpdate = vm.update;
    else vm.update = vm.__lmgQaUpdate;
    vm.setAim('lmg', 0);
    vm.draw('lmg', 1);
    let remaining = fraction;
    while (remaining > 0) {
      const step = Math.min(1 / 60, remaining);
      vm.__lmgQaUpdate.call(vm, step, { ads: 0, sway: 0, speed: 0 });
      remaining -= step;
    }
    vm.update = () => {};
  }, fraction);
  ...
  for (const fraction of [0.20, 0.55, 0.85]) { await pose('shoot', fraction); await snap(`shoot-${Math.round(fraction * 100)}`); }
  // Fases medidas na fonte MGX5: tampa aberta, caixa fora, cinto na bandeja, tampa fechando.
  for (const fraction of [0.25, 0.45, 0.65, 0.80]) { await pose('reload_tactical', fraction); await snap(`reload_tactical-${Math.round(fraction * 100)}`); }
  for (const fraction of [0.25, 0.50, 0.70, 0.80]) { await pose('reload_empty', fraction); await snap(`reload_empty-${Math.round(fraction * 100)}`); }
  await pose('inspect', 0.50); await snap('inspect-50');
  // ADS de ombro conservador; sockets permanecem medidos para muzzle/sight.
  await pose('idle', 0);
  await page.evaluate(() => {
    const vm = window.__authoredVm;
    vm.update = vm.__lmgQaUpdate || vm.update;
    vm.setAim('lmg', 1);
  ...
  await snap('ads');
  await page.close();
  console.log(`LMG_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-lmg-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
