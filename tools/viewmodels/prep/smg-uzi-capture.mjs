#!/usr/bin/env node
// Evidência no jogo real para o candidato UZI. O diretório padrão é privado e
...
const OUT = path.resolve(process.env.CSBRASIL_VM_EVIDENCE_DIR
  || '/Users/ruben/csbrasil-private-assets/generated/viewmodels-catalog-final/evidence/smg-uzi-20260914');
const errors = [];
const records = [];
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmready=smg&vmweapon=uzi&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('uzi'))) throw new Error(`${aspect}: UZI não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('uzi')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `uzi-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('uzi');
  ...
  const pose = async (clipName, fraction) => page.evaluate(({ clipName, fraction }) => {
    ...
    const entry = vm.entry('uzi');
    if (vm.__uziQaUpdate) vm.update = vm.__uziQaUpdate;
    vm.setAim('uzi', 0);
    ...
    if (!vm.__uziQaUpdate) vm.__uziQaUpdate = vm.update;
    vm.update = () => {};
  }, { clipName, fraction });
  const drawAt = async (fraction) => page.evaluate((fraction) => {
    const vm = window.__authoredVm;
    if (!vm.__uziQaUpdate) vm.__uziQaUpdate = vm.update;
    else vm.update = vm.__uziQaUpdate;
    vm.setAim('uzi', 0);
    vm.draw('uzi', 1);
    let remaining = fraction;
    while (remaining > 0) {
      const step = Math.min(1 / 60, remaining);
      vm.__uziQaUpdate.call(vm, step, { ads: 0, sway: 0, speed: 0 });
      remaining -= step;
    }
    vm.update = () => {};
  }, fraction);
  ...
  await page.evaluate(() => {
    const vm = window.__authoredVm;
    vm.update = vm.__uziQaUpdate || vm.update;
    vm.setAim('uzi', 1);
  ...
  await snap('ads');
  await page.close();
  console.log(`UZI_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-uzi-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
