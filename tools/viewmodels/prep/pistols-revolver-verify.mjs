...
check(gltf.animations.length === required.length + (clips.has('ads') ? 1 : 0), `catálogo inesperado de clips (${[...clips.keys()]})`);
...
const medeContatoDedos = (document, mutante = '') => {
  ...
  cena.getObjectByName('RIG_WEAPON_REVOLVER')?.traverse((mesh) => {
  ...
  if (!triangulos.length) throw new Error('contato Revólver .38 sem triângulos de arma');
...
console.log(`VM_PISTOL_REVOLVER=${JSON.stringify({ ok: failures.length === 0, file, bytes: bytes.length, sha256: cfg.sha256, clips: required, metrics, contatoIdleMm, triangulosMangaProximal, mutants, failures })}`);
