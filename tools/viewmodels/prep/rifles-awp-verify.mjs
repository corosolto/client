#!/usr/bin/env node
// Gate causal do candidato AWP. O asset permanece
...
function inspect(gltf) {
  ...
  const replacement = scene.getObjectByName('MINT_WEAPON_AWP_REPLACEMENT_MAG');
  ...
  check(!!rifleBone && !!magBone && !!replacementBone, 'bones mecânicos da família AWP ausentes');
  // O glTF expande vértices por normais/UV; as contagens pós-round-trip
  // congelam as ilhas obtidas da AWP pública soldada.
  check((gun?.geometry?.attributes?.position?.count || 0) === 6122,
  `corpo não corresponde à malha AWP pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((mag?.geometry?.attributes?.position?.count || 0) === 294,
    `carregador AWP não corresponde ao volume integral (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((replacement?.geometry?.attributes?.position?.count || 0) === 294,
    `carregador de reposição AWP divergente (${replacement?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((bolt?.geometry?.attributes?.position?.count || 0) === 78,
    `alavanca do ferrolho AWP divergente (${bolt?.geometry?.attributes?.position?.count || 0} vértices)`);
  ...
  for (const name of REQUIRED) {
    ...
    metrics[name] = {
      ...
      boltExcursion: bolt ? +trackExcursion(name, 'Bolt_metarig.position').toFixed(4) : null,
  ...
  for (const name of ['reload_tactical', 'reload_empty']) {
    check(Math.max(metrics[name]?.magExcursion || 0, metrics[name]?.replacementExcursion || 0) >= 0.45,
      `${name}: carregadores não percorrem remoção e encaixe do AWP`);
  ...
  check(metrics.shoot?.boltExcursion >= 0.08, 'shoot não cicla a alavanca real do ferrolho');
  check(metrics.reload_tactical?.boltExcursion <= 0.001, 'reload_tactical não deveria ciclar o ferrolho');
  check(metrics.reload_empty?.boltExcursion >= 0.08, 'reload_empty sem ciclo do ferrolho');
  check(metrics.reload_tactical?.handToMagMin <= 0.30, 'reload_tactical: mão esquerda não alcança o pente');
  check(metrics.reload_empty?.handToMagMin <= 0.30, 'reload_empty: mão esquerda não alcança o pente');
  ...
  if (gun && muzzle && sight) {
    ...
    check(Math.abs(Math.max(localDimensions.x, localDimensions.y, localDimensions.z) - 1.15) <= 0.04,
      `comprimento visual AWP ${Math.max(localDimensions.x, localDimensions.y, localDimensions.z).toFixed(4)} fora de 1,15 m`);
...
await mutant('ferrolho-vazio-congelado', (copy) => {
...
console.log(`VM_RIFLE_AWP=${JSON.stringify({ ok: failures.length === 0, file, bytes: bytes.length,
