#!/usr/bin/env node
// Evidência no jogo real para o candidato carabina. O diretório padrão é privado e
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmweapon=carbine&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('carbine'))) throw new Error(`${aspect}: carabina não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('carbine')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `carbine-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('carbine');
  ...
  for (const fraction of [0.32, 0.51, 0.70]) {
    await pose('reload_tactical', fraction); await snap(`reload_tactical-${fraction * 100}`);
  }
  for (const fraction of [0.32, 0.51, 0.70, 0.88]) {
    await pose('reload_empty', fraction); await snap(`reload_empty-${fraction * 100}`);
  }
  ...
  await snap('ads');
  await page.close();
  console.log(`carabina_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-rifle-carbine-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
