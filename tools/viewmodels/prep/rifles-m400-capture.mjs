#!/usr/bin/env node
// Evidência no jogo real para o candidato M400. O diretório padrão é privado e
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmweapon=m400&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('m400'))) throw new Error(`${aspect}: M400 não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('m400')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `m400-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('m400');
  ...
  // A M400 tem luneta: no jogo real o scope cobre o viewmodel. Primeiro congela
  // a pose física autoral para julgar eixo óptico/enquadramento; depois registra
  // separadamente a cobertura real da luneta.
  await pose('idle', 0);
  await page.evaluate(() => {
    const vm = window.__authoredVm;
    vm.setAim('m400', 1);
  ...
  await snap('ads');
  await page.evaluate(() => {
    delete window.__authoredVm.setAim;
    delete window.__authoredVm.update;
    window.__game._scope(true, true);
  });
  await page.waitForFunction(() => window.__game?.player?.scoped
    && window.__vmPrecisionQa?.state()?.visible?.scopeCovered, null, { timeout: 5000 });
  // O estado de visibilidade muda dentro do tick; aguarda três quadros completos
  // antes da captura para não registrar o framebuffer anterior, mesmo no SwiftShader.
  await page.evaluate(() => new Promise((resolve) => requestAnimationFrame(() =>
    requestAnimationFrame(() => requestAnimationFrame(resolve)))));
  await page.waitForTimeout(200);
  await snap('scope');
  await page.close();
  console.log(`M400_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-rifle-m400-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
