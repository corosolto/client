#!/usr/bin/env node
// Evidência no jogo real para o candidato AWP. O diretório padrão é privado e
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmweapon=awp&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('awp'))) throw new Error(`${aspect}: AWP não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('awp')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `awp-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('awp');
  ...
  // A AWP tem luneta: no jogo real o scope cobre o viewmodel. Primeiro congela
  ...
  await snap('scope');
  await page.close();
  console.log(`AWP_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-rifle-awp-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
