...
GENERAL = INT / 'public/private-assets/viewmodels/shared/general-runtime.glb'
...
def main():
    import sys as _sys
    argv = _sys.argv
    mutante_peca = '--mutante-peca' in argv
    ...
    for arma, plano in PLANOS.items():
        ...
        T_align = np.array(json.loads((F / f'alinhamento-{arma}.json').read_text())
        ...
        bone_idx = rig_root if mutante_peca else idx[peca_cfg['bone']]
        ...
        if plano['clip_proc']:
            ...
            for bone_nome in ['Clip', 'CartridgeClip0', 'CartridgeClip1', 'CartridgeClip2',
                ...
                if bone_nome == 'Clip':
                    ...
                    r = max(min(ext[1], ext[2]) * 0.45, 0.0035)
                    comp = max(ext[0], 0.030) * 0.9
            ...
            (F / f'clip-ancoras-{arma}.json').write_text(json.dumps(clip_stats, indent=1) + '\n')
        ...
        if plano.get('mag_autoral'):
            ...
            _sys.modules.setdefault('alinh', _alinh)
            ...
            arq_of = F / f'c-ofensores-{arma}.json'
        ...
        while len(json_bytes) % 4:
            json_bytes += b' '
        out = F / ('mutante-peca' if mutante_peca else f'{arma}-baked-runtime.glb')
        if mutante_peca:
            out = F / f'{arma}-baked-mutante-peca.glb'
    ...
    (F / 'build-resumo.json').write_text(json.dumps(resumo, indent=1) + '\n')
