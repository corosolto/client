#!/usr/bin/env node
// Evidência no jogo real para o candidato Tavor. O diretório padrão é privado e
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmweapon=tavor&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('tavor'))) throw new Error(`${aspect}: Tavor não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('tavor')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `tavor-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('tavor');
  ...
  for (const fraction of [0.40, 0.70]) { await pose('reload_tactical', fraction); await snap(`reload_tactical-${fraction * 100}`); }
  for (const fraction of [0.40, 0.70, 0.82]) { await pose('reload_empty', fraction); await snap(`reload_empty-${fraction * 100}`); }
  ...
  await snap('ads');
  await page.close();
  console.log(`Tavor_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-rifle-tavor-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
