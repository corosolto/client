...
const out = path.join(root, 'artifacts/viewmodels/prep/rifles/m4-reload-final-zcode');
...
const reloadBolt = sample(clips.reload_tactical, 62 / 30);
...
// The bolt event must be a real press: at t=2.067 s the thumb is on the paddle,
// and the press travel between 2.02 s and the bolt event is visible in the world
// position of thumb_03_l.
const thumbAt = t => { sample(clips.reload_tactical, t); const m = object('thumb_03_l').matrixWorld.elements; return [m[12], m[13], m[14]]; };
const thumbBefore = thumbAt(2.0), thumbAtBolt = thumbAt(62 / 30);
const pressTravelMm = Math.hypot(...thumbAtBolt.map((v, i) => v - thumbBefore[i])) * 1000;
const terminalTrack = clips.reload_tactical.tracks.find(track => track.name === 'pinky_01_l.quaternion');
...
const report = {
  ...
  cuffMorphWeights: { idle: idle.weights, reloadFrame20: reloadMid.weights, reloadBolt: reloadBolt.weights, reloadEnd: reloadEnd.weights },
  ...
  boltPressTravelMm: Math.round(pressTravelMm * 1000) / 1000,
...
// Structure and values against the C1 GLB: everything the final round did not
// intentionally touch must be identical; the touched left-arm/finger tracks are
// reported, and only their timing is required to match.
...
const LEFT_AUTHORED = /^(index|middle|ring|pinky)_0[123]_l\.(quaternion|position)$|^(hand_l|lowerarm_l|upperarm_l|lowerarm_twist_01_l)\.(quaternion|position)$|reload_cuff_cover_l\.scale\.x|GEO_FP_SK_Cloth_01\.morphTargetInfluences|GEO_FP_SK_Cloth_01\.weights|Cloth.*morph|^(thumb_0[123]_l)\.quaternion$|^MINT_WEAPON_M4_MAG\.position$/;
let delta = 0, checked = 0, relaxed = 0, relaxedMax = 0;
...
for (const sourceClip of original.animations) {
  ...
  for (const originalTrack of sourceClip.tracks) {
    ...
    if (!target) throw Error(`Track missing: ${originalTrack.name}`);
    if (sourceClip.name === 'reload_tactical' && LEFT_AUTHORED.test(originalTrack.name)) {
      // The glTF optimizer resamples changed curves; presence and timing span
      // are all that can be required of a track this round re-authored.
      if (Math.abs(target.times[target.times.length - 1] - originalTrack.times[originalTrack.times.length - 1]) > 1e-6) throw Error(`Track span changed: ${originalTrack.name}`);
      relaxedMax = Math.max(relaxedMax, maxDiff([...originalTrack.values], [...target.values]));
      relaxed++;
      continue;
    }
    const d = maxDiff([...originalTrack.values], [...target.values]);
    if (d > 1e-5) changedProtected.push(`${sourceClip.name}/${originalTrack.name}:${d.toFixed(4)}`);
    delta = Math.max(delta, d);
    checked++;
  }
}
report.protectedTracks = { checked, relaxed, maxValueDelta: delta, pass: delta < 1e-5, changedProtected,
  sourceSha256: createHash('sha256').update(source).digest('hex'), relaxedTracksMaxValueDelta: relaxedMax };
report.pass = Object.values(report.returnToIdleMaxMatrixDelta).every(v => v.start < 1e-5 && v.end < 1e-5) && report.clips.reload_tactical.morphWeightTracks === 1 && Math.max(...reloadMid.weights) > .5 && Math.max(...reloadEnd.weights) < 1e-5 && Math.max(...Object.values(endMeshDelta)) < 1e-5 && mutationRejected && report.protectedTracks.pass && pressTravelMm > 1.;
