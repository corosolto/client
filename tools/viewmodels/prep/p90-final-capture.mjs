#!/usr/bin/env node
// Evidência no jogo real para o candidato P90. O diretório padrão é privado e
...
const OUT = path.resolve(process.env.CSBRASIL_VM_EVIDENCE_DIR
  || '/Users/ruben/csbrasil-private-assets/generated/viewmodels-catalog-final/evidence/p90-20260914');
const errors = [];
const records = [];
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmready=p90&vmweapon=p90&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  await page.goto(`${BASE}/?${query}`, { waitUntil: 'domcontentloaded', timeout: 180000 });
  await page.addStyleTag({ content: 'astro-dev-toolbar,#vm-precision-qa,#vm-debug-badge,.tutorial-overlay{display:none!important}' });
  await page.waitForFunction(() => window.__game?.state === 'live' && window.__vmPrecisionQa, null, { timeout: 180000 });
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('p90'))) throw new Error(`${aspect}: P90 não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('p90')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `p90-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('p90');
  ...
  const pose = async (clipName, fraction) => page.evaluate(({ clipName, fraction }) => {
    ...
    const entry = vm.entry('p90');
    if (vm.__p90QaUpdate) vm.update = vm.__p90QaUpdate;
    vm.setAim('p90', 0);
    ...
    if (!vm.__p90QaUpdate) vm.__p90QaUpdate = vm.update;
    vm.update = () => {};
  }, { clipName, fraction });
  const drawAt = async (fraction) => page.evaluate((fraction) => {
    const vm = window.__authoredVm;
    if (!vm.__p90QaUpdate) vm.__p90QaUpdate = vm.update;
    else vm.update = vm.__p90QaUpdate;
    vm.setAim('p90', 0);
    vm.draw('p90', 1);
    let remaining = fraction;
    while (remaining > 0) {
      const step = Math.min(1 / 60, remaining);
      vm.__p90QaUpdate.call(vm, step, { ads: 0, sway: 0, speed: 0 });
      remaining -= step;
    }
    vm.update = () => {};
  }, fraction);
  ...
  // `inspect` é esparso (raiz do pacote); amostrá-lo ainda sobre o idle evita
  // herdar a última pose de ossos da recarga no mixer determinístico de QA.
  await pose('inspect', 0.50); await snap('inspect-50');
  for (const fraction of [0.30, 0.62]) { await pose('reload_tactical', fraction); await snap(`reload_tactical-${fraction * 100}`); }
  for (const fraction of [0.30, 0.62, 0.82]) { await pose('reload_empty', fraction); await snap(`reload_empty-${fraction * 100}`); }
  // ADS de ombro conservador. Os sockets continuam medidos no produto, mas a
  // auto-colimação foi rejeitada porque lança o cano para fora do quadro.
  await pose('idle', 0);
  await page.evaluate(() => {
    const vm = window.__authoredVm;
    vm.update = vm.__p90QaUpdate || vm.update;
    vm.setAim('p90', 1);
  ...
  await snap('ads');
  await page.close();
  console.log(`P90_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-p90-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
