"""Reautora a AKM pública sobre o rig KINEMATION aprovado.
...
pente curvo completo e a alavanca real de retenção são separados por topologia;
a fonte não contém charging handle separável, portanto nenhum é inventado.
...
if digest(m4_source) != M4_SHA or digest(akm_source) != AKM_SHA:
    raise RuntimeError("fonte M4/AKM ausente ou divergente")
...
# A fonte AKM e o pacote AR já apontam a boca para -X. A escala do objeto M4
# faz parte das ações herdadas, então a malha entra corrigida por essa constante.
...
release = bpy.data.objects.new("MINT_MAG_RELEASE_AKM", release_mesh)
...
report = {
    ...
    "weapon": "akm",
    "ready": False,
    "sources": {"arHandsActions": M4_SHA, "body": AKM_SHA},
    "selection": {
    ...
    "mechanism": "complete curved rock-in magazine and real magazine-catch press over KINEMATION rig; source has no separable charging handle",
}
(output_dir / "build.json").write_text(json.dumps(report, indent=2) + "\n")
print("AKM_KINEMATION_OK " + json.dumps(report, separators=(",", ":")))
