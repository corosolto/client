...
const cfg = manifest.candidates.famas;
...
function inspect(gltf) {
  ...
  const gun = scene.getObjectByName('MINT_WEAPON_FAMAS');
  const mag = scene.getObjectByName('MINT_WEAPON_FAMAS_MAG');
  const handle = scene.getObjectByName('MINT_CHARGING_FAMAS');
  ...
  check(!!gun, 'corpo MINT_WEAPON_FAMAS ausente');
  ...
  // O exportador divide seis vértices na borda de material; a fonte restante
  // continua sendo exatamente corpo = 6532 - 472 - 138 antes do round trip.
  check((gun?.geometry?.attributes?.position?.count || 0) === 5928,
  `corpo não corresponde à malha FAMAS pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((mag?.geometry?.attributes?.position?.count || 0) === 472,
    `carregador FAMAS não corresponde ao volume integral (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((handle?.geometry?.attributes?.position?.count || 0) === 138,
    `comando lateral FAMAS divergente (${handle?.geometry?.attributes?.position?.count || 0} vértices)`);
  ...
  const sample = (name, count = 30) => {
    ...
    for (let index = 0; index <= count; index += 1) {
      ...
      rows.push({
        ...
        handle: handle?.getWorldPosition(new THREE.Vector3()).applyMatrix4(gunInv),
  ...
  check(metrics.reload_empty?.handleExcursion >= 0.03, 'reload_empty sem ciclo do comando superior');
  check(metrics.reload_tactical?.handToMagMin <= 0.16, 'reload_tactical: mão esquerda não alcança o pente traseiro');
  check(metrics.reload_empty?.handToMagMin <= 0.16, 'reload_empty: mão esquerda não alcança o pente traseiro');
  ...
  if (gun && muzzle && sight) {
    ...
    check(Math.abs(Math.max(dimensions.x, dimensions.y, dimensions.z) - 0.6612) <= 0.04,
      `comprimento visual FAMAS ${Math.max(dimensions.x, dimensions.y, dimensions.z).toFixed(4)} fora de 0,6612 m`);
...
await mutant('sem-mag', (copy) => { copy.scene.getObjectByName('MINT_WEAPON_FAMAS_MAG')?.removeFromParent(); });
await mutant('mag-congelado', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_WEAPON_FAMAS_MAG.'))) {
...
await mutant('comando-congelado', (copy) => {
  ...
  for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_CHARGING_FAMAS.'))) {
    const stride = track.values.length / track.times.length;
    for (let i = stride; i < track.values.length; i += 1) track.values[i] = track.values[i % stride];
  }
});
await mutant('corpo-trocado', (copy) => {
  copy.scene.getObjectByName('MINT_WEAPON_FAMAS').geometry = copy.scene.getObjectByName('MINT_WEAPON_FAMAS_MAG').geometry;
});
await mutant('inspect-parado', (copy) => {
  const clip = copy.animations.find((item) => item.name === 'inspect');
  for (const track of clip.tracks.filter((item) => /^(MINT_WEAPON_FAMAS|RIG_FP_ARMS|VM_PACKAGE_FAMAS)\./.test(item.name))) {
    const stride = track.values.length / track.times.length;
    for (let i = stride; i < track.values.length; i += 1) track.values[i] = track.values[i % stride];
  }
});

console.log(`VM_RIFLE_FAMAS=${JSON.stringify({ ok: failures.length === 0, file, bytes: bytes.length,
