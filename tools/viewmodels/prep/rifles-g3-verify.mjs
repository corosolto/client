#!/usr/bin/env node
// Gate causal do candidato G3. O asset permanece
...
function inspect(gltf) {
  ...
  const replacement = scene.getObjectByName('MINT_WEAPON_G3_REPLACEMENT_MAG');
  ...
  check(!!gun, 'corpo MINT_WEAPON_G3 ausente');
  ...
  check(!!rifleBone && !!magBone && !!replacementBone, 'bones mecânicos da família G3 ausentes');
  // O glTF expande vértices por normais/UV; as contagens pós-round-trip
  // congelam as três ilhas obtidas da G3 pública soldada.
  check((gun?.geometry?.attributes?.position?.count || 0) === 6860,
  `corpo não corresponde à malha G3 pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((mag?.geometry?.attributes?.position?.count || 0) === 52,
    `carregador G3 não corresponde ao volume reto integral (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((replacement?.geometry?.attributes?.position?.count || 0) === 52,
    `carregador de reposição G3 divergente (${replacement?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((release?.geometry?.attributes?.position?.count || 0) === 130,
    `trava do pente G3 divergente (${release?.geometry?.attributes?.position?.count || 0} vértices)`);
  ...
  for (const name of ['reload_tactical', 'reload_empty']) {
    check(Math.max(metrics[name]?.magExcursion || 0, metrics[name]?.replacementExcursion || 0) >= 0.45,
      `${name}: carregadores não percorrem remoção e encaixe do G3`);
  ...
  check(metrics.reload_tactical?.handToMagMin <= 0.27, 'reload_tactical: mão esquerda não alcança o pente reto');
  check(metrics.reload_empty?.handToMagMin <= 0.27, 'reload_empty: mão esquerda não alcança o pente reto');
  ...
  if (gun && muzzle && sight) {
    ...
    check(Math.abs(Math.max(localDimensions.x, localDimensions.y, localDimensions.z) - 1.02) <= 0.04,
      `comprimento visual G3 ${Math.max(localDimensions.x, localDimensions.y, localDimensions.z).toFixed(4)} fora de 1,02 m`);
...
console.log(`VM_RIFLE_G3=${JSON.stringify({ ok: failures.length === 0, file, bytes: bytes.length,
