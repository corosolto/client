#!/usr/bin/env node
/** Gate causal da G3 reautorada sobre o rig KINEMATION comum. */
...
const cfg = manifest.candidates.g3;
const file = path.join(ASSET_ROOT, cfg.file);
const failures = [];
const check = (condition, message) => { if (!condition) failures.push(message); };

if (!fs.existsSync(file)) {
  console.log(`VM_RIFLE_G3=${JSON.stringify({ ok: false, failures: [`produto ausente em ${file}`] })}`);
  process.exit(1);
}
...
function inspect(gltf) {
  ...
  const gun = scene.getObjectByName('MINT_WEAPON_G3');
  const mag = scene.getObjectByName('MINT_WEAPON_G3_MAG');
  const release = scene.getObjectByName('MINT_MAG_RELEASE_G3');
  const muzzle = scene.getObjectByName('SOCKET_MINT_MUZZLE');
  const sight = scene.getObjectByName('SOCKET_MINT_SIGHT');
  localCheck(Boolean(gun), 'corpo MINT_WEAPON_G3 ausente');
  localCheck(Boolean(mag), 'pente reto separado ausente');
  localCheck(Boolean(release), 'botões reais de retenção ausentes');
  ...
  localCheck((gun?.geometry?.attributes?.position?.count || 0) === 6855,
    `corpo não corresponde à G3 pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((mag?.geometry?.attributes?.position?.count || 0) === 52,
    `pente integral divergente (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((release?.geometry?.attributes?.position?.count || 0) === 130,
    `botões de retenção divergentes (${release?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck(!scene.getObjectByName('MINT_CHARGING_G3'),
    'produto inventa charging handle que a fonte não separa');
  if (gun) {
    ...
    localCheck(Math.abs(visualLength - 1.02) <= 0.004,
      `comprimento visual ${visualLength.toFixed(4)} fora de 1,02 m`);
  }
  ...
  const sample = (name, count = 40) => {
    ...
    if (!clip || !gun || !mag || !release) return [];
    mixer.stopAllAction();
...
check(cfg.ready === false, 'G3 precisa permanecer ready:false');
...
await mutant('sem-mag', (copy) => { copy.scene.getObjectByName('MINT_WEAPON_G3_MAG')?.removeFromParent(); });
await mutant('mag-congelado', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_WEAPON_G3_MAG.'))) {
...
await mutant('sem-trava', (copy) => { copy.scene.getObjectByName('MINT_MAG_RELEASE_G3')?.removeFromParent(); });
await mutant('trava-congelada', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_MAG_RELEASE_G3.'))) {
      const stride = track.values.length / track.times.length;
      for (let index = stride; index < track.values.length; index += 1) track.values[index] = track.values[index % stride];
    }
  }
...
await mutant('corpo-trocado', (copy) => {
  copy.scene.getObjectByName('MINT_WEAPON_G3').geometry = copy.scene.getObjectByName('MINT_WEAPON_G3_MAG').geometry;
});
for (const actionName of ['inspect', 'equip_rifle']) {
  await mutant(`${actionName}-parado`, (copy) => {
    ...
    for (const track of clip.tracks.filter((item) => item.name.startsWith('VM_PACKAGE_G3.'))) {
      const stride = track.values.length / track.times.length;
      for (let index = stride; index < track.values.length; index += 1) track.values[index] = track.values[index % stride];
    }
  });
...
console.log(`VM_RIFLE_G3=${JSON.stringify({ ok: failures.length === 0, file,
