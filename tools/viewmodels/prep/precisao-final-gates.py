...
BASELINE = json.loads((F / 'donor-baseline.json').read_text())
...
def main():
    ...
    for arma, familia in FAMILIA.items():
        j, b = ler(F / f'{arma}-baked-runtime.glb')
        ...
        jn, bn = ler(RAIZ.parent / 'vm-astra-pistol' / 'public/private-assets/viewmodels' / fam_base / f'{fam_base}-runtime.glb')
        gaps_n = gate_T(jn, bn)
        ...
        jm, bm = ler(F / f'{arma}-baked-mutante-peca.glb')
        ...
        # C
        cc = gate_C_ml(arma, j, b, dump=F / f'c-ofensores-{arma}.json')
        ...
        gates[arma] = g
        (F / 'gates.json').write_text(json.dumps(gates, indent=1, default=float) + '\n')
