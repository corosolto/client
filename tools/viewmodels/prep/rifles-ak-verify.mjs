#!/usr/bin/env node
/** Gate causal da AK reautorada sobre o rig KINEMATION comum (receita rifles-ak-final.py). */
...
const cfg = manifest.candidates.ak;
const file = path.join(ASSET_ROOT, cfg.file);
const failures = [];
const check = (condition, message) => { if (!condition) failures.push(message); };

if (!fs.existsSync(file)) {
  console.log(`VM_RIFLE_AK=${JSON.stringify({ ok: false, failures: [`produto ausente em ${file}`] })}`);
  process.exit(1);
}
...
function inspect(gltf) {
  ...
  const gun = scene.getObjectByName('MINT_WEAPON_AK');
  const mag = scene.getObjectByName('MINT_WEAPON_AK_MAG');
  const release = scene.getObjectByName('MINT_MAG_RELEASE_AK');
  const handle = scene.getObjectByName('MINT_CHARGING_AK');
  const muzzle = scene.getObjectByName('SOCKET_MINT_MUZZLE');
  const sight = scene.getObjectByName('SOCKET_MINT_SIGHT');
  localCheck(Boolean(gun), 'corpo MINT_WEAPON_AK ausente');
  localCheck(Boolean(handle), 'manivela real do ferrolho ausente');
  localCheck(Boolean(mag), 'pente curvo separado ausente');
  localCheck(Boolean(release), 'alavanca real de retenção ausente');
  ...
  localCheck((gun?.geometry?.attributes?.position?.count || 0) === 4959,
    `corpo não corresponde à AK pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((mag?.geometry?.attributes?.position?.count || 0) === 896,
    `pente integral divergente (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((release?.geometry?.attributes?.position?.count || 0) === 61,
    `trava do pente divergente (${release?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((handle?.geometry?.attributes?.position?.count || 0) === 107,
    `manivela divergente (${handle?.geometry?.attributes?.position?.count || 0} vértices)`);
  if (gun) {
    ...
    localCheck(Math.abs(visualLength - 0.88) <= 0.004,
      `comprimento visual ${visualLength.toFixed(4)} fora de 0,88 m`);
  }
  ...
  const sample = (name, count = 40) => {
    ...
    if (!clip || !gun || !mag || !release || !handle) return [];
    mixer.stopAllAction();
    ...
    for (let index = 0; index <= count; index += 1) {
      ...
      rows.push({
        ...
        release: release.getWorldPosition(new THREE.Vector3()).sub(gunWorld),
        handle: handle.getWorldPosition(new THREE.Vector3()).sub(gunWorld),
      });
    }
    action.stop();
    mixer.update(0);
  ...
  for (const name of REQUIRED) {
    ...
    metrics[name] = {
      ...
      releaseExcursion: +excursion(rows, 'release').toFixed(4),
      ...
      endRelease: +endpoint(rows, 'release').toFixed(4),
      handleExcursion: +excursion(rows, 'handle').toFixed(4),
      endHandle: +endpoint(rows, 'handle').toFixed(4),
    };
  }
  ...
  for (const name of ['reload_tactical', 'reload_empty']) {
    ...
    localCheck(metrics[name]?.releaseExcursion >= 0.004, `${name} não pressiona a trava do pente`);
    localCheck(metrics[name]?.endRelease <= 0.01, `${name} não devolve a trava do pente`);
  }
  localCheck(metrics.reload_empty?.handleExcursion >= 0.03 && metrics.reload_empty?.endHandle <= 0.005,
    'reload_empty não puxa a manivela e devolve');
  localCheck((metrics.reload_tactical?.handleExcursion ?? 1) <= 0.002, 'reload_tactical não deveria mexer a manivela');
...
check(cfg.ready === false, 'produto K da AK aguarda nova aprovação do dono (ready:false no manifesto)');
...
await mutant('sem-mag', (copy) => { copy.scene.getObjectByName('MINT_WEAPON_AK_MAG')?.removeFromParent(); });
await mutant('mag-congelado', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_WEAPON_AK_MAG.'))) {
...
await mutant('sem-trava', (copy) => { copy.scene.getObjectByName('MINT_MAG_RELEASE_AK')?.removeFromParent(); });
await mutant('trava-congelada', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_MAG_RELEASE_AK.'))) {
      const stride = track.values.length / track.times.length;
      for (let index = stride; index < track.values.length; index += 1) track.values[index] = track.values[index % stride];
    }
  }
});
await mutant('manivela-congelada', (copy) => {
  const clip = copy.animations.find((item) => item.name === 'reload_empty');
  for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_CHARGING_AK.'))) {
    const stride = track.values.length / track.times.length;
    for (let index = stride; index < track.values.length; index += 1) track.values[index] = track.values[index % stride];
  }
});
...
await mutant('corpo-trocado', (copy) => {
  copy.scene.getObjectByName('MINT_WEAPON_AK').geometry = copy.scene.getObjectByName('MINT_WEAPON_AK_MAG').geometry;
});
for (const actionName of ['inspect', 'equip_rifle']) {
  await mutant(`${actionName}-parado`, (copy) => {
    ...
    for (const track of clip.tracks.filter((item) => item.name.startsWith('VM_PACKAGE_AK.'))) {
      const stride = track.values.length / track.times.length;
      for (let index = stride; index < track.values.length; index += 1) track.values[index] = track.values[index % stride];
    }
  });
...
console.log(`VM_RIFLE_AK=${JSON.stringify({ ok: failures.length === 0, file,
