#!/usr/bin/env node
// Evidência no jogo real para o candidato MP5. O diretório padrão é privado e
...
const OUT = path.resolve(process.env.CSBRASIL_VM_EVIDENCE_DIR
  || '/Users/ruben/csbrasil-private-assets/generated/viewmodels-catalog-final/evidence/smg-mp5-20260914');
const errors = [];
const records = [];
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmready=mp5&vmweapon=mp5&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('mp5'))) throw new Error(`${aspect}: MP5 não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('mp5')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `mp5-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('mp5');
  ...
  const pose = async (clipName, fraction) => page.evaluate(({ clipName, fraction }) => {
    ...
    const entry = vm.entry('mp5');
    if (vm.__mp5QaUpdate) vm.update = vm.__mp5QaUpdate;
    vm.setAim('mp5', 0);
    ...
    if (!vm.__mp5QaUpdate) vm.__mp5QaUpdate = vm.update;
    vm.update = () => {};
  }, { clipName, fraction });
  const drawAt = async (fraction) => page.evaluate((fraction) => {
    const vm = window.__authoredVm;
    if (!vm.__mp5QaUpdate) vm.__mp5QaUpdate = vm.update;
    else vm.update = vm.__mp5QaUpdate;
    vm.setAim('mp5', 0);
    vm.draw('mp5', 1);
    let remaining = fraction;
    while (remaining > 0) {
      const step = Math.min(1 / 60, remaining);
      vm.__mp5QaUpdate.call(vm, step, { ads: 0, sway: 0, speed: 0 });
      remaining -= step;
    }
    vm.update = () => {};
  }, fraction);
  ...
  await page.evaluate(() => {
    const vm = window.__authoredVm;
    vm.update = vm.__mp5QaUpdate || vm.update;
    vm.setAim('mp5', 1);
  ...
  await snap('ads');
  await page.close();
  console.log(`MP5_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-mp5-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
