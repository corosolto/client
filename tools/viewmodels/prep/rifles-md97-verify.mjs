...
const cfg = manifest.candidates.md97;
...
function inspect(gltf) {
  ...
  const gun = scene.getObjectByName('MINT_WEAPON_MD97');
  const mag = scene.getObjectByName('MINT_WEAPON_MD97_MAG');
  ...
  check(!!gun, 'corpo MINT_WEAPON_MD97 ausente');
  ...
  check((gun?.geometry?.attributes?.position?.count || 0) >= 6400
    && (gun?.geometry?.attributes?.position?.count || 0) <= 6500,
  `corpo não corresponde à malha MD97 pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((mag?.geometry?.attributes?.position?.count || 0) === 48,
    `carregador MD97 não corresponde aos dois segmentos medidos (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  ...
  if (gun && muzzle && sight) {
    ...
    check(Math.abs(Math.max(dimensions.x, dimensions.y, dimensions.z) - 0.9135) <= 0.04,
      `comprimento visual MD97 ${Math.max(dimensions.x, dimensions.y, dimensions.z).toFixed(4)} fora de 1,05×0,87`);
...
await mutant('sem-mag', (copy) => { copy.scene.getObjectByName('MINT_WEAPON_MD97_MAG')?.removeFromParent(); });
await mutant('mag-congelado', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_WEAPON_MD97_MAG.'))) {
...
await mutant('corpo-trocado', (copy) => {
  copy.scene.getObjectByName('MINT_WEAPON_MD97').geometry = copy.scene.getObjectByName('MINT_WEAPON_MD97_MAG').geometry;
});
await mutant('inspect-parado', (copy) => {
  const clip = copy.animations.find((item) => item.name === 'inspect');
  for (const track of clip.tracks.filter((item) => /^(MINT_WEAPON_MD97|RIG_FP_ARMS|VM_PACKAGE_MD97)\./.test(item.name))) {
    const stride = track.values.length / track.times.length;
    for (let i = stride; i < track.values.length; i += 1) track.values[i] = track.values[i % stride];
  }
});

console.log(`VM_RIFLE_MD97=${JSON.stringify({ ok: failures.length === 0, file, bytes: bytes.length,
