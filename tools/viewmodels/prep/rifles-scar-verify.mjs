...
const cfg = manifest.candidates.scar;
...
function inspect(gltf) {
  ...
  const gun = scene.getObjectByName('MINT_WEAPON_SCAR');
  const mag = scene.getObjectByName('MINT_WEAPON_SCAR_MAG');
  const handle = scene.getObjectByName('MINT_CHARGING_SCAR');
  ...
  check(!!gun, 'corpo MINT_WEAPON_SCAR ausente');
  ...
  check((gun?.geometry?.attributes?.position?.count || 0) === 6707,
  `corpo não corresponde à malha SCAR pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((mag?.geometry?.attributes?.position?.count || 0) === 236,
    `carregador SCAR não corresponde ao volume integral (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((handle?.geometry?.attributes?.position?.count || 0) === 81,
    `comando lateral SCAR divergente (${handle?.geometry?.attributes?.position?.count || 0} vértices)`);
  ...
  check(metrics.shoot?.handleExcursion >= 0.03, 'shoot sem ciclo do comando lateral');
  ...
  if (gun && muzzle && sight) {
    ...
    check(Math.abs(Math.max(dimensions.x, dimensions.y, dimensions.z) - 0.90) <= 0.04,
      `comprimento visual SCAR ${Math.max(dimensions.x, dimensions.y, dimensions.z).toFixed(4)} fora de 0,90 m`);
...
await mutant('sem-mag', (copy) => { copy.scene.getObjectByName('MINT_WEAPON_SCAR_MAG')?.removeFromParent(); });
await mutant('mag-congelado', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_WEAPON_SCAR_MAG.'))) {
...
await mutant('comando-congelado', (copy) => {
  ...
  for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_CHARGING_SCAR.'))) {
    const stride = track.values.length / track.times.length;
    for (let i = stride; i < track.values.length; i += 1) track.values[i] = track.values[i % stride];
  }
});
await mutant('corpo-trocado', (copy) => {
  copy.scene.getObjectByName('MINT_WEAPON_SCAR').geometry = copy.scene.getObjectByName('MINT_WEAPON_SCAR_MAG').geometry;
});
await mutant('inspect-parado', (copy) => {
  const clip = copy.animations.find((item) => item.name === 'inspect');
  for (const track of clip.tracks.filter((item) => /^(MINT_WEAPON_SCAR|RIG_FP_ARMS|VM_PACKAGE_SCAR)\./.test(item.name))) {
    const stride = track.values.length / track.times.length;
    for (let i = stride; i < track.values.length; i += 1) track.values[i] = track.values[i % stride];
  }
});

console.log(`VM_RIFLE_SCAR=${JSON.stringify({ ok: failures.length === 0, file, bytes: bytes.length,
