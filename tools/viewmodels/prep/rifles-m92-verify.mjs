#!/usr/bin/env node
// Gate causal do candidato M92. O asset permanece
...
function inspect(gltf) {
  ...
  const replacement = scene.getObjectByName('MINT_WEAPON_M92_REPLACEMENT_MAG');
  ...
  const handleBone = scene.getObjectByName('Bolt_metarig');
  ...
  check(!!rifleBone && !!magBone && !!replacementBone && !!handleBone,
    'bones mecânicos da família AK ausentes');
  // O glTF expande vértices por normais/UV; as contagens pós-round-trip
  // congelam as três ilhas obtidas da M92 pública soldada.
  check((gun?.geometry?.attributes?.position?.count || 0) === 6637,
  `corpo não corresponde à malha M92 pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((mag?.geometry?.attributes?.position?.count || 0) === 102,
    `carregador M92 não corresponde ao volume integral (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((replacement?.geometry?.attributes?.position?.count || 0) === 102,
    `carregador de reposição M92 divergente (${replacement?.geometry?.attributes?.position?.count || 0} vértices)`);
  check((handle?.geometry?.attributes?.position?.count || 0) === 127,
    `comando lateral M92 divergente (${handle?.geometry?.attributes?.position?.count || 0} vértices)`);
  ...
  const sample = (name, count = 30) => {
    ...
    for (let index = 0; index <= count; index += 1) {
      ...
      const bolt = handleBone.getWorldPosition(new THREE.Vector3());
      rows.push({
        mag: installed.sub(rifle),
        replacement: fresh.sub(rifle),
        handle: bolt.sub(rifle),
        gun: rifle,
  ...
  for (const name of REQUIRED) {
    ...
    metrics[name] = {
      ...
      handleExcursion: handle ? +excursion(rows, 'handle').toFixed(4) : null,
  ...
  check(metrics.reload_empty?.handleExcursion >= 0.20, 'reload_empty sem ciclo do comando lateral');
  check(Math.abs((metrics.reload_empty?.handleExcursion || 0)
    - (metrics.reload_tactical?.handleExcursion || 0)) >= 0.005,
  'reload_empty não diferencia o ciclo do comando da recarga tática');
  ...
  if (gun && muzzle && sight) {
    ...
    check(Math.abs(Math.max(localDimensions.x, localDimensions.y, localDimensions.z) - 0.684) <= 0.04,
      `comprimento visual M92 ${Math.max(localDimensions.x, localDimensions.y, localDimensions.z).toFixed(4)} fora de 0,684 m`);
...
await mutant('sem-mag', (copy) => { copy.scene.getObjectByName('MINT_WEAPON_M92_MAG')?.removeFromParent(); });
...
await mutant('comando-congelado', (copy) => {
  ...
  for (const track of clip.tracks.filter((item) => item.name.startsWith('Bolt_metarig.'))) {
    const stride = track.values.length / track.times.length;
    for (let i = stride; i < track.values.length; i += 1) track.values[i] = track.values[i % stride];
  }
});
...
console.log(`VM_RIFLE_M92=${JSON.stringify({ ok: failures.length === 0, file, bytes: bytes.length,
