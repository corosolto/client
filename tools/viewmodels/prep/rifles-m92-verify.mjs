#!/usr/bin/env node
/** Gate causal da M92 reautorada sobre o rig KINEMATION comum. */
...
const cfg = manifest.candidates.m92;
const file = path.join(ASSET_ROOT, cfg.file);
const failures = [];
const check = (condition, message) => { if (!condition) failures.push(message); };

if (!fs.existsSync(file)) {
  console.log(`VM_RIFLE_M92=${JSON.stringify({ ok: false, failures: [`produto ausente em ${file}`] })}`);
  process.exit(1);
}
...
function inspect(gltf) {
  ...
  const gun = scene.getObjectByName('MINT_WEAPON_M92');
  const mag = scene.getObjectByName('MINT_WEAPON_M92_MAG');
  const handle = scene.getObjectByName('MINT_CHARGING_M92');
  const muzzle = scene.getObjectByName('SOCKET_MINT_MUZZLE');
  const sight = scene.getObjectByName('SOCKET_MINT_SIGHT');
  localCheck(Boolean(gun), 'corpo MINT_WEAPON_M92 ausente');
  localCheck(Boolean(mag), 'carregador curvo separado ausente');
  localCheck(Boolean(handle), 'alavanca lateral real separada ausente');
  ...
  localCheck((gun?.geometry?.attributes?.position?.count || 0) === 6635,
    `corpo não corresponde à M92 pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((mag?.geometry?.attributes?.position?.count || 0) === 102,
    `pente integral divergente (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((handle?.geometry?.attributes?.position?.count || 0) === 127,
    `alavanca lateral divergente (${handle?.geometry?.attributes?.position?.count || 0} vértices)`);
  if (gun) {
    ...
    localCheck(Math.abs(visualLength - 0.684) <= 0.004,
      `comprimento visual ${visualLength.toFixed(4)} fora de 0,684 m`);
  }
  ...
  localCheck(metrics.reload_empty?.handleExcursion >= 0.03,
    'reload_empty não aciona a alavanca lateral');
  localCheck(metrics.reload_empty?.endHandle <= 0.01,
    'reload_empty não devolve a alavanca lateral');
  localCheck(metrics.reload_tactical?.handleExcursion <= 0.005,
    'reload_tactical move a alavanca lateral indevidamente');
...
check(cfg.ready === false, 'M92 precisa permanecer ready:false');
...
await mutant('sem-maos', (copy) => {
  const handMesh = copy.scene.getObjectByName('GEO_FP_SK_Hand');
  handMesh?.removeFromParent();
});
await mutant('sem-shoot', (copy) => {
  copy.animations = copy.animations.filter((clip) => clip.name !== 'shoot');
});
await mutant('sem-mag', (copy) => {
  copy.scene.getObjectByName('MINT_WEAPON_M92_MAG')?.removeFromParent();
});
await mutant('mag-congelado', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_WEAPON_M92_MAG.'))) {
...
await mutant('sem-alavanca', (copy) => {
  copy.scene.getObjectByName('MINT_CHARGING_M92')?.removeFromParent();
});
await mutant('alavanca-congelada', (copy) => {
  const clip = copy.animations.find((item) => item.name === 'reload_empty');
  for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_CHARGING_M92.'))) {
    const stride = track.values.length / track.times.length;
    for (let index = stride; index < track.values.length; index += 1) {
      track.values[index] = track.values[index % stride];
    }
...
await mutant('sem-sight', (copy) => {
  copy.scene.getObjectByName('SOCKET_MINT_SIGHT')?.removeFromParent();
});
await mutant('sem-camera', (copy) => { copy.cameras.length = 0; });
await mutant('corpo-trocado', (copy) => {
  copy.scene.getObjectByName('MINT_WEAPON_M92').geometry = copy.scene.getObjectByName('MINT_WEAPON_M92_MAG').geometry;
});
for (const actionName of ['inspect', 'equip_rifle']) {
  await mutant(`${actionName}-parado`, (copy) => {
    ...
    for (const track of clip.tracks.filter((item) => item.name.startsWith('VM_PACKAGE_M92.'))) {
      const stride = track.values.length / track.times.length;
      for (let index = stride; index < track.values.length; index += 1) {
        track.values[index] = track.values[index % stride];
      }
...
console.log(`VM_RIFLE_M92=${JSON.stringify({ ok: failures.length === 0, file,
