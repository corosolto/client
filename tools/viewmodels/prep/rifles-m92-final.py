"""Reautora a Zastava M92 sobre o rig KINEMATION aprovado.

A versão anterior reutilizava o golden público da AK e, apesar de ter ações e
mãos visíveis, mantinha um esqueleto `*_metarig` sem qualquer osso em comum com
o rig das outras armas. Esta receita abre o checkpoint final da M4, preserva o
rig, as três camadas de mão e a gramática de seis ações, e troca apenas as
malhas da arma.

A M92 pública é soldada antes do corte. O pente curvo completo e a alavanca
lateral real são selecionados por ilhas topológicas congeladas. O pente herda
as duas recargas KINEMATION; a alavanca recebe curso próprio somente na recarga
vazia. Produtos continuam fora do Git.
"""
...
if digest(m4_source) != M4_SHA or digest(m92_source) != M92_SHA:
    raise RuntimeError("fonte M4/M92 ausente ou divergente")
...
# A M92 pública aponta a boca para +X; o pacote KINEMATION aponta para -X.
...
handle = bpy.data.objects.new("MINT_CHARGING_M92", handle_mesh)
...
if handle.animation_data:
    ...
    for track in handle.animation_data.nla_tracks:
        track.mute = False
bpy.context.view_layer.update()
...
report = {
    ...
    "weapon": "m92",
    "ready": False,
    "sources": {"arHandsActions": M4_SHA, "body": M92_SHA},
    "selection": {
    ...
    "mechanism": "complete curved rock-in magazine and real right-side charging handle on empty reload over KINEMATION rig",
}
(output_dir / "build.json").write_text(json.dumps(report, indent=2) + "\n")
print("M92_KINEMATION_OK " + json.dumps(report, separators=(",", ":")))
