#!/usr/bin/env node
// Evidência no jogo real para o candidato shotgun. O diretório padrão é privado e
...
const OUT = path.resolve(process.env.CSBRASIL_VM_EVIDENCE_DIR
  || '/Users/ruben/csbrasil-private-assets/generated/viewmodels-catalog-final/evidence/shotgun-20260914');
const errors = [];
const records = [];
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmready=shotgun&vmweapon=shotgun&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('shotgun'))) throw new Error(`${aspect}: shotgun não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('shotgun')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `shotgun-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('shotgun');
  ...
  const pose = async (clipName, fraction) => page.evaluate(({ clipName, fraction }) => {
    ...
    const entry = vm.entry('shotgun');
    if (vm.__shotgunQaUpdate) vm.update = vm.__shotgunQaUpdate;
    vm.setAim('shotgun', 0);
    ...
    if (!vm.__shotgunQaUpdate) vm.__shotgunQaUpdate = vm.update;
    vm.update = () => {};
  }, { clipName, fraction });
  const drawAt = async (fraction) => page.evaluate((fraction) => {
    const vm = window.__authoredVm;
    if (!vm.__shotgunQaUpdate) vm.__shotgunQaUpdate = vm.update;
    else vm.update = vm.__shotgunQaUpdate;
    vm.setAim('shotgun', 0);
    vm.draw('shotgun', 1);
    let remaining = fraction;
    while (remaining > 0) {
      const step = Math.min(1 / 60, remaining);
      vm.__shotgunQaUpdate.call(vm, step, { ads: 0, sway: 0, speed: 0 });
      remaining -= step;
    }
    vm.update = () => {};
  }, fraction);
  ...
  await pose('reload_start', 0.50); await snap('reload_start-50');
  for (const fraction of [0.25, 0.50, 0.75]) { await pose('reload_loop', fraction); await snap(`reload_loop-${fraction * 100}`); }
  await pose('reload_end', 0.50); await snap('reload_end-50');
  await pose('inspect', 0.50); await snap('inspect-50');
  // ADS de ombro conservador; sockets permanecem medidos para muzzle/sight.
  await pose('idle', 0);
  await page.evaluate(() => {
    const vm = window.__authoredVm;
    vm.update = vm.__shotgunQaUpdate || vm.update;
    vm.setAim('shotgun', 1);
  ...
  await snap('ads');
  await page.close();
  console.log(`SHOTGUN_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-shotgun-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
