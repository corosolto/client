"""Export the reload-final M4 candidate (GLB + camera + sockets) and its evidence.

Frames 0/13/20/25/30/35/43/45/54/62/70/72 are rendered in 3:2 and 16:9 from the
game camera, plus close-ups from both sides and material-id passes.  The whole
cycle is rendered frame by frame in both aspects for the offline video.
...
CLIP = 'reload_tactical'
CRITICAL = [0, 13, 20, 25, 30, 35, 43, 45, 54, 62, 70, 72]
...
left_finger_ids = {finger: [v.index for v in glove.data.vertices
...
def surface_distance_stats(points, ids, tree):
    ...
    return {'vertices': len(distances), 'min_mm': round(distances[0], 3),
            ...
            'within_5mm': sum(d <= 5 for d in distances)}


measure = {'clip': CLIP, 'frames': frames, 'per_frame': []}
prev = None
for frame in range(frames + 1):
    ...
    row = {'frame': frame, 't': round(frame / lib.FPS, 5),
           'phase': build['clips'][CLIP]['log'][frame]['phase'],
           ...
           'hand_l': list(bone_head('hand_l')), 'gun_origin': list(gun.matrix_world.translation),
           'mag_origin': list(mag.matrix_world.translation),
           'thumb_tip_to_bolt_release_mm': round((bone_tail('thumb_03_l')
                                                  - bpy.data.objects['bolt_release'].matrix_world.translation).length * 1000, 2)}
    if prev:
        row['step_mm'] = {k: round((Vector(row[k]) - Vector(prev[k])).length * 1000, 2)
                          for k in ('hand_l', 'gun_origin', 'mag_origin')}
    measure['per_frame'].append(row)
    prev = row
first, last = measure['per_frame'][0], measure['per_frame'][-1]
at('idle', 0)
idle_ref = {'hand_l': list(bone_head('hand_l')), 'gun_origin': list(gun.matrix_world.translation),
            'mag_origin': list(mag.matrix_world.translation)}
...
measure['max_step_mm'] = {k: max(r['step_mm'][k] for r in measure['per_frame'][1:]) for k in ('hand_l', 'gun_origin', 'mag_origin')}
...
measure['magazine_events'] = {'first_frame_off_seat': moving[0] if moving else None,
                              'first_frame_reseated': next((f for f in seated if moving and f > moving[0]), None),
                              'game_events_frames': {k: round(v * frames, 2) for k, v in lib.GAME_RELOAD_EVENTS.items()}}
measure['bolt_press'] = {str(r['frame']): r['thumb_tip_to_bolt_release_mm'] for r in measure['per_frame'] if r['frame'] in CRITICAL}
(evidence / 'measurements.json').write_text(json.dumps(measure, indent=1) + '\n')

# ---------------------------------------------------------------- camera-space dumps
SAMPLE_FRAMES = {'reload_tactical': CRITICAL, 'idle': [0]}
...
for name in ('idle', CLIP):
    ...
    for frame in SAMPLE_FRAMES.get(name, [0]):
        at(name, frame)
        dg = bpy.context.evaluated_depsgraph_get()
...
for frame in CRITICAL:
    ...
    render(evidence / f'f{frame:03d}-close-hand.png', 768, 576, frame,
           location=hand + toward_camera * .28 + Vector((0, 0, .03)), target=hand, lens=40)
    render(evidence / f'f{frame:03d}-close-hand-opposite.png', 768, 576, frame,
           location=hand - toward_camera * .28 + Vector((0, 0, .03)), target=hand, lens=40)
# material-id: cloth red, glove blue, skin green, gun grey, magazine orange
...
for frame in CRITICAL:
    ...
    hand = bone_head('hand_l')
    toward_camera = (home_location - hand).normalized()
    render(evidence / f'f{frame:03d}-material-id.png', 768, 576, frame,
           location=hand + toward_camera * .28 + Vector((0, 0, .03)), target=hand, lens=40)
    render(evidence / f'f{frame:03d}-material-id-opposite.png', 768, 576, frame,
           location=hand - toward_camera * .28 + Vector((0, 0, .03)), target=hand, lens=40)
# continuous strips, every frame, both aspects, for the offline video
scene.render.engine = 'CYCLES'
scene.cycles.samples = 6
for label, size in (('strip', (384, 256)), ('strip-16x9', (384, 216))):
    strip = evidence / label
    strip.mkdir(exist_ok=True)
    for frame in range(frames + 1):
        render(strip / f'f{frame:03d}.png', size[0], size[1], frame)
...
summary = {'glb': str(glb.relative_to(inv.ROOT)), 'glb_sha256': glb_sha, 'glb_bytes': glb.stat().st_size,
           'clips': {n: frames_of[n] / lib.FPS for n in frames_of},
           'return_to_idle_mm': measure['return_to_idle_mm'], 'max_step_mm': measure['max_step_mm'],
           'magazine_events': measure['magazine_events'], 'bolt_press': measure['bolt_press'],
           'critical_frames': CRITICAL}
(evidence / 'summary.json').write_text(json.dumps(summary, indent=1) + '\n')
print('M4_RELOAD_FINAL_EXPORT', json.dumps(summary, indent=None))
