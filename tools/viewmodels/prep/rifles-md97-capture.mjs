#!/usr/bin/env node
// Evidência no jogo real para o candidato MD97. O diretório padrão é privado e
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmweapon=md97&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('md97'))) throw new Error(`${aspect}: MD97 não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('md97')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `md97-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('md97');
  ...
  await pose('idle', 0); await page.evaluate(() => window.__authoredVm.setAim('md97', 1));
  await page.waitForTimeout(350); await snap('ads');
  await page.close();
  console.log(`MD97_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-rifle-md97-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
