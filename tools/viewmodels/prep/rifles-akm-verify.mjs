#!/usr/bin/env node
/** Gate causal da AKM reautorada sobre o rig KINEMATION comum. */
...
if (!fs.existsSync(file)) {
  console.log(`VM_RIFLE_AKM=${JSON.stringify({ ok: false, failures: [`produto ausente em ${file}`] })}`);
  process.exit(1);
}
...
function inspect(gltf) {
  ...
  localCheck(Boolean(gun), 'corpo MINT_WEAPON_AKM ausente');
  ...
  localCheck((gun?.geometry?.attributes?.position?.count || 0) === 6295,
    `corpo não corresponde à AKM pública (${gun?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((mag?.geometry?.attributes?.position?.count || 0) === 624,
    `pente integral divergente (${mag?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck((release?.geometry?.attributes?.position?.count || 0) === 140,
    `alavanca de retenção divergente (${release?.geometry?.attributes?.position?.count || 0} vértices)`);
  localCheck(!scene.getObjectByName('MINT_CHARGING_AKM'),
...
check(cfg.ready === false, 'AKM precisa permanecer ready:false');
...
await mutant('mag-congelado', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_WEAPON_AKM_MAG.'))) {
...
await mutant('sem-trava', (copy) => { copy.scene.getObjectByName('MINT_MAG_RELEASE_AKM')?.removeFromParent(); });
await mutant('trava-congelada', (copy) => {
  for (const clip of copy.animations.filter((item) => /^reload_/.test(item.name))) {
    for (const track of clip.tracks.filter((item) => item.name.startsWith('MINT_MAG_RELEASE_AKM.'))) {
      const stride = track.values.length / track.times.length;
      for (let index = stride; index < track.values.length; index += 1) track.values[index] = track.values[index % stride];
    }
  }
...
for (const actionName of ['inspect', 'equip_rifle']) {
  await mutant(`${actionName}-parado`, (copy) => {
    ...
    for (const track of clip.tracks.filter((item) => item.name.startsWith('VM_PACKAGE_AKM.'))) {
      const stride = track.values.length / track.times.length;
      for (let index = stride; index < track.values.length; index += 1) track.values[index] = track.values[index % stride];
    }
  });
...
console.log(`VM_RIFLE_AKM=${JSON.stringify({ ok: failures.length === 0, file,
