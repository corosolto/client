...
PACKS = os.environ.get(
    'CSBRASIL_VM_DONOR_ROOT',
    '/Users/ruben/csbrasil-private-assets/generated/viewmodels',
)
...
def valor_accessor(doc, bin_data, indice, linha=0):
    """Lê uma linha float de accessor (suficiente para TRS das animações)."""
    acc = doc['accessors'][indice]
    if acc.get('componentType') != 5126:
        raise ValueError(f'accessor {indice}: componente não-float')
    ncomp = {'SCALAR': 1, 'VEC2': 2, 'VEC3': 3, 'VEC4': 4}[acc['type']]
    bv = doc['bufferViews'][acc['bufferView']]
    stride = bv.get('byteStride', ncomp * 4)
    off = bv.get('byteOffset', 0) + acc.get('byteOffset', 0) + linha * stride
    return list(struct.unpack_from('<' + 'f' * ncomp, bin_data, off))


def pose_primeira_chave(doc, bin_data, nome='idle'):
    """TRS dos nós na primeira chave do clipe (idle@t0 no runtime)."""
    anim = next((a for a in doc.get('animations', []) if a.get('name') == nome), None)
    if not anim:
        raise ValueError(f'animação ausente: {nome}')
    pose = {}
    for canal in anim.get('channels', []):
        alvo = canal.get('target', {})
        no = alvo.get('node')
        caminho = alvo.get('path')
        if no is None or caminho not in ('translation', 'rotation', 'scale'):
            continue
        sampler = anim['samplers'][canal['sampler']]
        pose.setdefault(no, {})[caminho] = valor_accessor(doc, bin_data, sampler['output'], 0)
    return pose


def mundo_do_no_pose(doc, indice, pose):
    """Matriz de mundo com overrides TRS de uma pose amostrada."""
    pais = {c: i for i, n in enumerate(doc['nodes']) for c in n.get('children', [])}
    cadeia = []
    atual = indice
    while True:
        cadeia.append(atual)
        if atual not in pais:
            break
        atual = pais[atual]
    total = identidade()
    for i in reversed(cadeia):
        no = dict(doc['nodes'][i])
        no.update(pose.get(i, {}))
        total = mul(total, trs_para_matriz(no))
    return total
...
def montar(arma, cfg):
    ...
    pose_idle = pose_primeira_chave(base_doc, base_bin, 'idle')
    ...
    sampler_map = reindexa(mini_doc.get('samplers', []), 'samplers')
    image_map = reindexa(mini_doc.get('images', []), 'images')
    texture_map = reindexa(mini_doc.get('textures', []), 'textures')
    base_bv_count = len(base_doc.get('bufferViews', []))
    ...
    for i, acc in enumerate(mini_doc.get('accessors', [])):
        ...
        if 'bufferView' in novo:
            novo['bufferView'] = base_bv_count + novo['bufferView']
        base_doc.setdefault('accessors', []).append(novo)
    base_doc.setdefault('bufferViews', []).extend(novos_buffer_views)
    for sampler in mini_doc.get('samplers', []):
        base_doc.setdefault('samplers', []).append(dict(sampler))
    for imagem in mini_doc.get('images', []):
        nova = json.loads(json.dumps(imagem))
        if 'bufferView' in nova:
            nova['bufferView'] = base_bv_count + nova['bufferView']
        base_doc.setdefault('images', []).append(nova)
    for textura in mini_doc.get('textures', []):
        nova = json.loads(json.dumps(textura))
        if 'source' in nova:
            nova['source'] = image_map[f'images:{nova["source"]}']
        if 'sampler' in nova:
            nova['sampler'] = sampler_map[f'samplers:{nova["sampler"]}']
        webp = nova.get('extensions', {}).get('EXT_texture_webp')
        if webp and 'source' in webp:
            webp['source'] = image_map[f'images:{webp["source"]}']
        base_doc.setdefault('textures', []).append(nova)

    def reindexa_texturas(valor, chave=''):
        if isinstance(valor, dict):
            out = {}
            for k, v in valor.items():
                if k == 'index' and chave.lower().endswith('texture'):
                    out[k] = texture_map[f'textures:{v}']
                else:
                    out[k] = reindexa_texturas(v, k)
            return out
        if isinstance(valor, list):
            return [reindexa_texturas(v, chave) for v in valor]
        return valor

    for mat in mini_doc.get('materials', []):
        base_doc.setdefault('materials', []).append(reindexa_texturas(mat))
    for ext in mini_doc.get('extensionsUsed', []):
        if ext not in base_doc.setdefault('extensionsUsed', []):
            base_doc['extensionsUsed'].append(ext)
    for ext in mini_doc.get('extensionsRequired', []):
        if ext not in base_doc.setdefault('extensionsRequired', []):
            base_doc['extensionsRequired'].append(ext)
    ...
    for nome_peca, nome_pai in cfg['pais'].items():
        ...
        # A Mint foi aferida contra as mãos na pose idle do Blender. O jogo
        # também aplica idle imediatamente; ancorar contra a bind pose produz
        # o deslocamento gigante visto nas capturas, embora os nomes e hashes
        # continuem corretos. Usa o mesmo referencial idle@primeira-chave.
        mundo_pai = mundo_do_no_pose(base_doc, indice_por_nome[nome_pai], pose_idle)
    ...
    mundo_pai = mundo_do_no_pose(base_doc, corpo_idx, pose_idle)
    ...
    relatorio['attachmentReference'] = 'idle@first-key'
    relatorio['recursosMint'] = {
        'materials': len(mini_doc.get('materials', [])),
        'textures': len(mini_doc.get('textures', [])),
        'images': len(mini_doc.get('images', [])),
    }
