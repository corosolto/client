#!/usr/bin/env node
// Evidência no jogo real para o candidato G3. O diretório padrão é privado e
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmweapon=g3&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('g3'))) throw new Error(`${aspect}: G3 não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('g3')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `g3-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('g3');
  ...
  // Define ADS explicitamente: alternar `player.scoped` torna a segunda resolução
  // dependente do estado inicial da partida e pode capturar hip-fire como "ads".
  await pose('idle', 0); await page.evaluate(() => window.__game._scope(true, true));
  await page.waitForFunction(() => window.__game?.player?.scoped
    && window.__authoredVm?.adsAmount > 0.9, null, { timeout: 5000 });
  await snap('ads');
  await page.close();
  console.log(`G3_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-rifle-g3-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
