#!/usr/bin/env node
// Evidência no jogo real para o candidato Revólver. O diretório padrão é privado e
...
const OUT = path.resolve(process.env.CSBRASIL_VM_EVIDENCE_DIR
  || '/Users/ruben/csbrasil-private-assets/generated/viewmodels-catalog-final/evidence/pistols-revolver-20260914');
const errors = [];
const records = [];
...
for (const [aspect, width, height] of [['3x2', 1440, 960], ['16x9', 1440, 810]]) {
  ...
  const query = 'debug=1&auto=P,mst&vmready=revolver&vmweapon=revolver38&map=piscina_treta&armaslazy=0&vmauthored=1&vmqa=precision';
  ...
  if (!await page.evaluate(() => window.__vmPrecisionQa.equip('revolver38'))) throw new Error(`${aspect}: Revólver não equipou`);
  await page.waitForFunction(() => window.__authoredVm?.entry?.('revolver38')?.mint?.active, null, { timeout: 120000 });
  await page.evaluate(() => {
  ...
  const snap = async (state) => {
    const file = path.join(OUT, `revolver38-${state}-${aspect}.png`);
    const runtime = await page.evaluate(() => {
      const entry = window.__authoredVm.entry('revolver38');
  ...
  const pose = async (clipName, fraction) => page.evaluate(({ clipName, fraction }) => {
    ...
    const entry = vm.entry('revolver38');
    if (vm.__revolver38QaUpdate) vm.update = vm.__revolver38QaUpdate;
    vm.setAim('revolver38', 0);
    ...
    if (!vm.__revolver38QaUpdate) vm.__revolver38QaUpdate = vm.update;
    vm.update = () => {};
  }, { clipName, fraction });
  const drawAt = async (fraction) => page.evaluate((fraction) => {
    const vm = window.__authoredVm;
    if (!vm.__revolver38QaUpdate) vm.__revolver38QaUpdate = vm.update;
    else vm.update = vm.__revolver38QaUpdate;
    vm.setAim('revolver38', 0);
    vm.draw('revolver38', 1);
    let remaining = fraction;
    while (remaining > 0) {
      const step = Math.min(1 / 60, remaining);
      vm.__revolver38QaUpdate.call(vm, step, { ads: 0, sway: 0, speed: 0 });
      remaining -= step;
    }
    vm.update = () => {};
  }, fraction);
  ...
  for (const fraction of [0.30, 0.55, 0.78]) { await pose('reload_empty', fraction); await snap(`reload_empty-${fraction * 100}`); }
  await pose('inspect', 0.50); await snap('inspect-50');
  // ADS físico com muzzle/sight medidos no produto baked.
  await pose('idle', 0);
  await page.evaluate(() => {
    const vm = window.__authoredVm;
    vm.update = vm.__revolver38QaUpdate || vm.update;
    vm.setAim('revolver38', 1);
  ...
  await snap('ads');
  await page.close();
  console.log(`Revólver_CAPTURE ${aspect} ok`);
}
await browser.close();
...
fs.writeFileSync(path.join(OUT, 'capture.json'), `${JSON.stringify({ schemaVersion: 1,
  kind: 'real-browser-revolver-capture', revision: execSync('git rev-parse HEAD').toString().trim(),
