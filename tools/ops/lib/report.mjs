...
function linhaSonda(nome, s) {
  ...
  if (s.sonda === 'assets') return `| assets no edge | ${s.faltando.length || s.conteudoErrado.length || s.semResposta.length ? `${s.faltando.length} 404 · ${s.conteudoErrado.length} corpo errado · ${s.semResposta.length} sem resposta` : 'ok'} | ${s.total} sondados · armas do ${s.registro?.origem === 'registro-servido' ? `weapons.js servido (${s.registro.armas})` : 'registro da árvore'} · p95 ${s.p95ms ?? '—'} ms · ${s.cacheHits} HIT |`;
