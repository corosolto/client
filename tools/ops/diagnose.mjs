...
export async function diagnosticar(flags, { log = () => {} } = {}) {
  ...
  if (flags.remoto) {
    ...
    if (boot.html?.status === 200) {
      ...
      sondas.assets = await sondaAssetsRemoto(flags.base, { raiz, versao: boot.versaoHtml || contexto.versaoLocal, compararTamanho: mesmaVersao, timeoutMs: flags.timeout });
      comandos.push(`[${quando}] GET Range bytes=0-15 em ${sondas.assets.total} assets de ${flags.base} (?v=${boot.versaoHtml || contexto.versaoLocal})`);
