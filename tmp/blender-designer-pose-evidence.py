"""Renderiza bind/walk/crouch/idle do Designer UX rigado para inspeção visual."""
import pathlib
import sys
...
scene.world = bpy.data.worlds.new('DesignerPoseWorld')
scene.world.color = (0.025, 0.03, 0.045)
bpy.ops.object.camera_add()
camera = bpy.context.object
camera.data.type = 'ORTHO'
camera.data.ortho_scale = max(span) * 1.15
camera.location = center + Vector((0, -4.0, 0))
...
for action_name, frame in [('idle', 20), ('walk', 15), ('crouch', 15)]:
    ...
    scene.render.filepath = str(out / f'designer-ux-{action_name}.png')
    bpy.ops.render.render(write_still=True)
print(f'DESIGNER_POSE_EVIDENCE={out}')
