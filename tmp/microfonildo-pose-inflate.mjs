...
const MICRO_BASE = process.env.MICRO_BASE || 'references/tv/microfonildo/3d/blender-v1';
const EXPLICIT_MODEL = arg('model');
const ANIMS = arg('anims', `${MICRO_BASE}/anims`);
const OUTPUT = arg('out', `${MICRO_BASE}/pose-inflate.json`);
const ALL_CLIPS = ['idle','walk','run','crouch','crouchwalk','jump','death','shoot','idle1h','walk1h','walkfire'];
const CLIPES = arg('clips') === 'all' ? ALL_CLIPS : (arg('clips')?.split(',').filter(Boolean) || ['walk', 'run', 'idle', 'crouchwalk']);
const AMOSTRAS = 8;
const hash = file => createHash('sha256').update(fs.readFileSync(file)).digest('hex');

if (EXPLICIT_MODEL) {
  if (!fs.existsSync(EXPLICIT_MODEL)) throw new Error(`modelo explícito ausente: ${EXPLICIT_MODEL}`);
  if (!fs.statSync(ANIMS).isDirectory()) throw new Error(`diretório explícito de clipes ausente: ${ANIMS}`);
  const missing = CLIPES.filter(clip => !fs.existsSync(`${ANIMS}/${clip}.glb`));
  if (missing.length) throw new Error(`clipes explícitos ausentes: ${missing.join(', ')}`);
  if (flag('require-all') && (CLIPES.length !== ALL_CLIPS.length || ALL_CLIPS.some(clip => !CLIPES.includes(clip)))) {
    throw new Error(`--require-all exige os ${ALL_CLIPS.length} clipes canônicos`);
  }
}
...
const alvos = EXPLICIT_MODEL ? [EXPLICIT_MODEL] : (positional.length ? positional : fs.readdirSync(CHARS).filter((f) => f.endsWith('.glb')).map((f) => f.replace('.glb', '')));
const out = [];
...
const provenance = EXPLICIT_MODEL ? {
  mode: 'explicit-no-fallback',
  model: path.resolve(EXPLICIT_MODEL),
  modelSha256: hash(EXPLICIT_MODEL),
  animDirectory: path.resolve(ANIMS),
  animations: Object.fromEntries(CLIPES.map(clip => [clip, { file: path.resolve(`${ANIMS}/${clip}.glb`), sha256: hash(`${ANIMS}/${clip}.glb`) }])),
} : { mode: 'legacy' };
fs.writeFileSync(OUTPUT, JSON.stringify({ gerado: new Date().toISOString(), amostras: AMOSTRAS, clipes: CLIPES, provenance, personagens: out }, null, 1));
console.log(`\n-> ${OUTPUT}`);
