...
|---|---:|---|
| Código do jogo | 38.132 linhas em 59 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Scripts do arnês | 236 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **117** importam Playwright, **41** importam gltf-transform e **5** importam meshoptimizer.
...
|---|---|---|---|---:|
| `praca_poderes` | Praça dos Três Poderes | rodadas | `map_brasilia.js` | 1.905 |
