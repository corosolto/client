...
|---|---:|---|
| Código do jogo | 53.066 linhas em 108 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.938** linhas | `wc -l public/js/game.js` |
| `main.js` | 3.589 linhas | `wc -l public/js/main.js` |
...
| Scripts do arnês | 509 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **204** importam Playwright, **81** importam gltf-transform e **10** importam meshoptimizer.
