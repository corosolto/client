...
|---|---:|---|
| Código do jogo | 41.248 linhas em 73 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.383** linhas | `wc -l public/js/game.js` |
...
| Props em GLB | 136 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **123** importam Playwright, **46** importam gltf-transform e **8** importam meshoptimizer.
...
**15 mapas registrados** — 2 abrem em rodadas e 13 em captura. `ctfMode` **abre** o mapa em captura, não prende: o jogador troca no menu (é a `MOD1`). Há 17 arquivos `map_*.js` em `public/js/` — arquivo no disco **não** implica mapa jogável.
