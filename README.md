...
| Jogo | ES modules vanilla, **zero build** | 72 arquivos |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **127** importam Playwright, **45** importam gltf-transform e **7** importam meshoptimizer.
