...
|---|---:|---|
| Código do jogo | 43.740 linhas em 65 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Scripts do arnês | 320 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **150** importam Playwright, **66** importam gltf-transform e **5** importam meshoptimizer.
