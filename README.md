...
|---|---:|---|
| Código do jogo | 42.811 linhas em 64 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.349** linhas | `wc -l public/js/game.js` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **147** importam Playwright, **62** importam gltf-transform e **5** importam meshoptimizer.
