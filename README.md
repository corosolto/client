...
|---|---:|---|
| Código do jogo | 46.965 linhas em 94 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.437** linhas | `wc -l public/js/game.js` |
...
| Scripts do arnês | 367 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
| Jogo | ES modules vanilla, **zero build** | 94 arquivos |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **143** importam Playwright, **57** importam gltf-transform e **10** importam meshoptimizer.
