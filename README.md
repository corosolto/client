...
|---|---:|---|
| Código do jogo | 42.903 linhas em 63 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.260** linhas | `wc -l public/js/game.js` |
| `main.js` | 2.817 linhas | `wc -l public/js/main.js` |
...
| Props em GLB | 160 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Scripts do arnês | 313 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Scripts de pipeline | 84 | `git ls-files 'tools/*.mjs' \| wc -l` |
...
| Jogo | ES modules vanilla, **zero build** | 63 arquivos |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **144** importam Playwright, **63** importam gltf-transform e **8** importam meshoptimizer.
...
| `amazonia` | Treta na Amazônia | **captura** | `map_amazonia.js` | 1.106 |
