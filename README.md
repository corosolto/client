...
|---|---:|---|
| Código do jogo | 44.354 linhas em 87 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Scripts do arnês | 325 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
| Jogo | ES modules vanilla, **zero build** | 87 arquivos |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **139** importam Playwright, **49** importam gltf-transform e **10** importam meshoptimizer.
