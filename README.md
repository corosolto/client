...
|---|---:|---|
| Código do jogo | 43.871 linhas em 66 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.559** linhas | `wc -l public/js/game.js` |
...
| Jogo | ES modules vanilla, **zero build** | 66 arquivos |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **158** importam Playwright, **68** importam gltf-transform e **5** importam meshoptimizer.
