...
|---|---:|---|
| Código do jogo | 40.912 linhas em 59 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.188** linhas | `wc -l public/js/game.js` |
| `main.js` | 2.781 linhas | `wc -l public/js/main.js` |
...
| Scripts do arnês | 293 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **142** importam Playwright, **58** importam gltf-transform e **5** importam meshoptimizer.
...
`package.json` tem **181 scripts**; o motivo de cada um mora em `SCRIPTS.md` (migrado das chaves `//nome` em 18/08/2026) — é onde está o porquê.
