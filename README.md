...
|---|---:|---|
| Código do jogo | 40.856 linhas em 58 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props em GLB | 135 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Scripts do arnês | 275 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **136** importam Playwright, **55** importam gltf-transform e **4** importam meshoptimizer.
...
`package.json` tem **168 scripts**. Vários trazem uma chave `//nome` logo acima com o motivo de existirem — é onde mora o porquê.
...
| `fy_campomorro` | Campo do Morro | **captura** | `map_campomorro.js` | 762 |
