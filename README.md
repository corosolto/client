...
|---|---:|---|
| Código do jogo | 46.862 linhas em 93 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props em GLB | 172 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Scripts do arnês | 364 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **143** importam Playwright, **58** importam gltf-transform e **10** importam meshoptimizer.
...
| `posto_treta` | Posto da Treta | **captura** | `map_posto.js` | 459 |
