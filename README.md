...
|---|---:|---|
| Código do jogo | 32.611 linhas em 47 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Scripts de pipeline | 56 | `git ls-files 'tools/*.mjs' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **112** importam Playwright, **40** importam gltf-transform e **5** importam meshoptimizer.
