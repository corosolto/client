...
|---|---:|---|
| Código do jogo | 40.249 linhas em 55 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Props em GLB | 137 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Scripts do arnês | 264 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **130** importam Playwright, **56** importam gltf-transform e **4** importam meshoptimizer.
