...
|---|---:|---|
| Código do jogo | 43.878 linhas em 66 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **159** importam Playwright, **68** importam gltf-transform e **5** importam meshoptimizer.
