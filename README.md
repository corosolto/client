...
|---|---:|---|
| Código do jogo | 52.365 linhas em 105 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Scripts do arnês | 497 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **202** importam Playwright, **80** importam gltf-transform e **10** importam meshoptimizer.
