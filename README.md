...
|---|---:|---|
| Código do jogo | 39.570 linhas em 64 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.356** linhas | `wc -l public/js/game.js` |
| `main.js` | 3.373 linhas | `wc -l public/js/main.js` |
...
| Props em GLB | 126 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **122** importam Playwright, **44** importam gltf-transform e **7** importam meshoptimizer.
