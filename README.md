...
|---|---:|---|
| Código do jogo | 41.104 linhas em 73 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.374** linhas | `wc -l public/js/game.js` |
| `main.js` | 3.385 linhas | `wc -l public/js/main.js` |
...
| Props em GLB | 134 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Mapas no registro | 15 | objeto `MAPS` de `maps.js` |
| Arnêses visuais em HTML | 15 | `git ls-files 'public/*.html' \| wc -l` |
| Scripts do arnês | 284 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Scripts de pipeline | 61 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Tarefas de entrada escritas | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Versão | `2.0.0-alpha.227` | `public/js/version.js` e `package.json` (batem) |
...
| Jogo | ES modules vanilla, **zero build** | 73 arquivos |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **128** importam Playwright, **45** importam gltf-transform e **7** importam meshoptimizer.
...
| Mapas no menu | 15 — 2 abrem em rodadas, **13 em captura** | `MAPS` / `ctfMode` |
...
|---|---|---|---|---:|
| `escadao` | Escadão (Morro) | **captura** | `map_escadao.js` | 1.317 |
...
**15 mapas registrados** — 2 abrem em rodadas e 13 em captura. `ctfMode` **abre** o mapa em captura, não prende: o jogador troca no menu (é a `MOD1`). Há 20 arquivos `map_*.js` em `public/js/` — arquivo no disco **não** implica mapa jogável.
