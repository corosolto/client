...
|---|---:|---|
| Código do jogo | 41.670 linhas em 60 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.345** linhas | `wc -l public/js/game.js` |
| `main.js` | 2.815 linhas | `wc -l public/js/main.js` |
...
| Scripts do arnês | 298 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Scripts de pipeline | 77 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Tarefas de entrada escritas | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Versão | `2.0.0-alpha.201` | `public/js/version.js` e `package.json` (batem) |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **142** importam Playwright, **57** importam gltf-transform e **5** importam meshoptimizer.
