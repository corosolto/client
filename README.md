...
|---|---:|---|
| Código do jogo | 40.552 linhas em 59 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Versão | `2.0.0-alpha.176` | `public/js/version.js` e `package.json` (batem) |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **143** importam Playwright, **58** importam gltf-transform e **5** importam meshoptimizer.
