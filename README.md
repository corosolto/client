...
|---|---:|---|
| Código do jogo | 39.760 linhas em 65 arquivos | `git ls-files public/js/*.js \| xargs wc -l` |
| `game.js` | **7.365** linhas | `wc -l public/js/game.js` |
| `main.js` | 3.375 linhas | `wc -l public/js/main.js` |
...
| Props em GLB | 128 | `git ls-files 'public/models/props/*.glb' \| wc -l` |
...
| Scripts do arnês | 250 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
...
Three.js sai de `public/vendor/three.module.js` (**sem CDN, sem npm no runtime**). Astro e Vercel de `package.json` + `astro.config.mjs` + `vercel.json`. Dos scripts de `tools/`, **118** importam Playwright, **45** importam gltf-transform e **8** importam meshoptimizer.
