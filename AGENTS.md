...
|---|---|---|---|
| `public/` | o **jogo** | 34 arquivos `.js`, 27.725 linhas · Three.js `r160` vendorizado | ES modules servidos crus, **zero build**, sem dependência de runtime |
| `src/` | o **site** | 18 páginas `.astro`, 19 rotas `/api` · Astro `^7.1.1` | framework é bem-vindo; `service_role` só no servidor |
| `tools/` | o **arnês** | 161 scripts em `tools/eval/`, 46 em `tools/` | node puro: sobe o jogo real sem browser |
...
`package.json` tem **85 scripts**. Vários trazem uma chave `//nome` logo acima com o motivo de existirem — é onde mora o porquê.
