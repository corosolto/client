...
export function buildPoolDay(scene, T) {
  ...
  return {
    ...
    sound: {
      bioma: 'indoor',
      /* `src` e `pos` de TRÊS componentes: é o que o soundscape.js lê (`l.src`,
         `loop.pos[2]`) e o que os outros 16 mapas declaram. As posições, raios e
         volumes são os da reconstrução da main; só o nome da chave e o eixo y vêm
         do lado da branch, senão o loop nunca carrega e a distância sai NaN. */
      loops: [
        { src: AMB_LOOPS.piscina, pos: [0, .5, 0], radius: 30, vol: 0.24 },
        { src: AMB_LOOPS.hum, pos: [SERVICE.axisX, 3, 0], radius: 18, vol: 0.08 },
      ],
    },
