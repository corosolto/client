...
export function attachMintWeapon(entry, weaponId) {
  ...
  if (!wrap) {
    ...
    // O fallback do pack permanece visível até a malha própria existir.
    if (!wrap) return null;
  ...
  // Só agora a arma própria existe: esconder o placeholder antes deste ponto
  // produziria mãos sem arma durante carregamento ou falha de asset.
  hidePackGun(entry);
