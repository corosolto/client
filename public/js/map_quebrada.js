...
export function buildQuebrada(scene, T) {
  ...
  // Muretas baixas protegem a chegada da travessa sem fechar os dois portões.
  for (const [mx, mz, ry] of [[-16.9, 25.1, -0.16], [16.9, 25.1, 0.16]]) {
    ...
    const ex = 1.4 * Math.abs(cos) + .24 * Math.abs(sin), ez = 1.4 * Math.abs(sin) + .24 * Math.abs(cos);
    Object.assign(colliders[colliders.length - 1], {
      minX: mx - ex, maxX: mx + ex, minZ: mz - ez, maxZ: mz + ez,
  ...
  // Bancos curtos desenham duas rotas laterais sem invadir o corredor central.
  ...
  // O placar fecha a leitura esportiva do fundo sem adicionar colisão.
  {
  ...
  // Contornos medidos com o raio real: arquibancada, ponto de ônibus e feira.
  for (const [x, z] of [[-15.7, 29.3], [-9.5, -9.1], [16.7, 43.9]]) if (!blocked(x, z, .38)) nodes.push({ x, z });

  // Varredura contínua evita arestas de A* atravessando quinas entre amostras.
  const segClear = (a, b) => {
    for (const c of colliders) {
      ...
      for (const [axis, min, max] of [['x', c.ry ? -c.hx : c.minX, c.ry ? c.hx : c.maxX], ['z', c.ry ? -c.hz : c.minZ, c.ry ? c.hz : c.maxZ]]) {
        ...
        if (Math.abs(delta) < 1e-9) {
          if (A[axis] <= min - .38 || A[axis] >= max + .38) { enter = 2; break; }
        } else {
